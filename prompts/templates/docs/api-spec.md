...
> **【分割告知 / 段階移行中】**
> 本テンプレートは Contract Task / Implementation Task の責務分離に伴い、以下の2テンプレートへ分割された。
>
> | 用途 | テンプレート | 担当Task |
> | ---- | ------------ | -------- |
> | API契約面（基本情報 / Request / Response / Error / Validation / 互換性 / OpenAPI反映方針） | `api-contract-spec.md` | Contract Task |
> | API実装面（処理フロー / 内部DTOマッピング / provider・consumer実装影響 / ログ / 結合テスト観点） | `api-implementation-spec.md` | Implementation Task |
