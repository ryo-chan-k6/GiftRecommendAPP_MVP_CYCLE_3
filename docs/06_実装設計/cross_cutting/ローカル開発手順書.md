...
| api | **実装済み（health + PUB-002 モジュール）** | `./scripts/dev/start-api.sh` / `pnpm dev:api` で **http://localhost:3001** 待受。前提: `pnpm install`。`GET /api/v1/health` は起動可。`POST /api/v1/recommendations`（API-PUB-002）はモジュール実装済みだが **Router 未マウント**（§10.4） |
...
補助確認（api 未マウント時の reco 単体）: API-INT-002 は body が `recommendationRequestId` + `recommendationRequest` ネスト形（[internal-reco-api.yaml](../../../packages/contracts/openapi/internal-reco-api.yaml)）。`X-Trace-Id` / `X-Request-Id` / `X-Internal-Api-Key` 必須。実値はログ・docs に出さない。
...
| `POST /api/v1/recommendations`（PUB-002） | HTTP **404** | `Cannot POST /api/v1/recommendations` |
...
本 Task では **OpenAPI / migration / Batch / Epic C 全面整備は out of scope**。縦串成功に必要な最小ギャップは次のとおり（test-data の item fixture 不足が主因ではない）。

| No | ギャップ | 事実 | 後続候補 |
| --: | -------- | ---- | -------- |
| 1 | api Router 未マウント | `apps/api/src/app/recommendations/**` は実装済みだが `createApp()`（`app.ts`）は health のみ。PR #1129 でマウントは epic_scope 外と明記 | API-PUB-002 Router マウント Task（`app.ts` に `POST /recommendations` を配線） |
| 2 | api Request 永続化が scaffold | `RecommendationRequestRepository` は `ScaffoldDbSession` 利用。実 PostgreSQL の `recommendation_request` へ書かない | 実 DB session 配線 Task（api DB 接続） |
| 3 | reco production pair_reader | `build_production_observability_modules` が `InMemoryPairMasterReader({friend,birthday})` のまま。DB の `pair_master`（boss×thanks 等）を参照しない | reco composition で Postgres pair_master reader へ置換 |
| 4 | reco config version / pipeline | Run Recorder が config version 存在確認で失敗。候補取得以降も MVP stub ports が主 | reco production ports 拡充・version 解決整合（別 Epic / Task） |
| 5 | test-data | item / feature / meaning / embedding は縦串 item 側として充足。`recommendation_request` seed 追加だけでは No.1〜4 を解消しない | 本 Task では **seed 拡充せず**、不足は後続 Task 明示で止める |

**成功条件（本 Task）:** 手順が再現可能に記載され、実行結果（404 / 既知 500 原因）と後続 Task 候補が PR・本節に残っていること。HTTP 200 の商品返却までは後続。
...
| PUB-002 が 404 | `createRecommendationsRouter` が `app.ts` 未マウント（§10.4.4 No.1）。health のみ起動している状態 |
...
| API-PUB-002 縦串 | §10.4 — Issue #1137。手順・実行結果・既知ギャップ（Router 未マウント等）を記録。HTTP 200 商品返却は後続 Task |
