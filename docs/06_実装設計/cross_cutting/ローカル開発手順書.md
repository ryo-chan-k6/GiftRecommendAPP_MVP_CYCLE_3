...
### 7.4 Object Storage bucket（`raw-products`）

接続方針 A（製品=Supabase Storage / S3 互換）。bucket 名正本は **`raw-products`**（private）。

| 項目 | 内容 |
| ---- | ---- |
| 定義正本 | `supabase/migrations/20260725120000_raw_products_storage_bucket.sql` |
| 方針正本 | [マイグレーション方針書 §9.2](../database/マイグレーション方針書.md) |
| env | `OBJECT_STORAGE_BUCKET=raw-products`（[`.env.example`](../../../.env.example)） |

**ローカル**

1. `./scripts/db/start-local.sh`（Storage 有効: `supabase/config.toml` `[storage] enabled = true`）
2. `./scripts/db/migrate-up.sh`（上記 migration で `raw-products` が作成される）
3. batch live 煙を行う場合のみ、ローカル向け S3 互換 endpoint / キーを `.env` の `OBJECT_STORAGE_*` に設定（**実値は commit しない**。キー発行手順は Hosted と同様に Dashboard / プロジェクト設定側）

**Hosted（Human）**

1. 対象 Supabase プロジェクトへ migration を適用する（正）。Dashboard で同名 private bucket を手動作成するのはフォールバック
2. Storage の **S3 互換** Access Key / Secret Key を発行し、GitHub Secrets / ローカル `.env` に設定する（リポジトリへ書かない）
3. `OBJECT_STORAGE_ENDPOINT` をプロジェクトの S3 互換 endpoint に合わせる（形式例は環境設計書 §19.7）

**live 疎通（#1617・明示フラグのみ）**
...
| 項目 | 内容 |
| ---- | ---- |
| 計画 / 結果 | [Supabase_Storage_S3互換疎通検証計画](../../90_PoC/外部API疎通検証/Supabase_Storage_S3互換疎通検証計画.md) / [結果](../../90_PoC/外部API疎通検証/Supabase_Storage_S3互換疎通検証結果.md) |
| 必須 env | `OBJECT_STORAGE_ENDPOINT` / `ACCESS_KEY` / `SECRET_KEY` / `BUCKET`（§19.7） |
| region（local） | `supabase status` の Storage Region（多くは `local`）。`--region local` を推奨。未指定時ハーネス既定は `us-east-1`（local でも疎通できる場合あり） |
| 禁止 | CI 既定 live、secret 実値の commit / PR 貼付 |
| 出力 | `scripts/batch/output-object-storage-live/`（Git 管理外） |
...
`PostgresRecommendationResultRepository`（021 header）と `PostgresRecommendationResultItemRepository`（022 item）を production に注入。DEFAULT InMemory 維持。Reason 永続は当時 out of scope（#1294）。
...
後続候補（当時）: API 応答 itemPrice マッピング確認／NG concept 除外精査／Reason 永続。

#### 10.4.10b Reason Postgres 永続後（2026-07-17 / Issue #1413）

`PostgresRecommendationReasonRepository`（023 / IF-DB-RECO-008）を production に注入。DEFAULT InMemory 維持。`reason_template` / `item_semantic` reader の Postgres 本実装は本 Task out of scope。

| 確認 | 結果 | 備考 |
| ---- | ---- | ---- |
| repository / composition UT | **通過想定**（CI / ローカル `pnpm --filter @gift-recommendation/reco test`） | |
| production `reason_generator.reason_repository` | **Postgres** | DEFAULT は InMemory |
| DB 確認 SQL（ローカル） | `SELECT recommendation_reason_id, recommendation_result_item_id, reason_summary FROM recommendation_reason ORDER BY created_at DESC LIMIT 5;` | PUB-002 実行後に行が増えること |
...
#### 10.4.14 レーン1f NG正規化 api 正本化（2026-07-15 / Issue #1324）

Contract（#1321 / PR #1322）で INT-002 `NgCondition` に `ngKeywords` / `ngCategories` を追加済み。本 Task で api が `ngText` から `ngKeywords` を派生して INT-002 へ渡し、reco `request_mapper` が domain へマップする。PreFilter は **request.ng_text を再抽出しない**（attribute hard_filter 経路の暫定抽出のみ残置）。

| 確認 | 結果 | 備考 |
| ---- | ---- | ---- |
| api ng-normalize UT | **通過** | 「アルコールはNG」→「アルコール」 |
| reco pre_hard_filter UT | **通過** | api 提供 keywords / ng_text 非再抽出 |
| OpenAPI / Orval | Contract 済み | #1322 |

後続候補: attribute 経路の暫定抽出整理／Reason 永続／Epic #1320 クローズ。

#### 10.4.15 レーン1e 手動 E2E（D1）（2026-07-15 / Issue #1331）

正本チェックリスト: [レーン1e_手動E2Eチェックリスト.md](./レーン1e_手動E2Eチェックリスト.md)

| 確認 | 結果 | 備考 |
| ---- | ---- | ---- |
| チェックリスト正本 | **作成** | S1〜S4 必須 / S5〜S6 推奨 |
| S1 結果あり UI | **pass** | SCR-002→003→004。件数1・¥4,320。`resultId=4314c45c-…` |
| S1 API | **pass** | HTTP 200 / `traceId=d1-manual-e2e-s1-001` / count=1 |
| S2 0件 | **pass**（再検証） | #1345 後: empty 200 + SCR-009 UI + 条件変更復帰（2026-07-16） |
| S3 エラー | **pass** | api 停止でエラー UI → 条件入力へ戻る |
| S4 再検索 | **pass** | 結果→SCR-002 |
| S5 / S6 | **pass** | 理由詳細 / alcohol NG |
| Residual | #1344 解消済 | #1345 empty / #1346 SCR-001 |

後続: D2（Playwright）／品質基盤。SCR-005〜007 回帰は §10.4.16。Reason Postgres 永続は §10.4.10b（#1412 / #1417）。

#### 10.4.16 SCR-005〜007 回帰 E2E（2026-07-17 / Issue #1419）

正本: [レーン1e_手動E2Eチェックリスト.md](./レーン1e_手動E2Eチェックリスト.md) §9

| 確認 | 結果 | 備考 |
| ---- | ---- | ---- |
| シナリオ定義（S5' / S7 / S8） | **作成** | points/detail・商品詳細・Feedback |
| S5' / S7 / S8 手動実行 | **pass** | `resultId=8723c5d6-…` / `traceId=scr-reg-e2e-s5-001`。詳細はチェックリスト §9.3 |

証跡: `ai-logs/experiments/2026-07-17-scr-post-mvp-regression-e2e.md`

後続: D2（Playwright）→ §10.4.17。

#### 10.4.17 Playwright 自動 E2E（D2）（2026-07-17 / Issue #1439）

| 項目 | 内容 |
| ---- | ---- |
| Epic / Task | #1438 / #1439 |
| 配置 | `apps/web/playwright.config.ts` / `apps/web/e2e/` |
| インストール | `pnpm --filter @gift-recommendation/web test:e2e:install` |
| 実行 | `pnpm --filter @gift-recommendation/web test:e2e` |
| 前提 | web :3000 / api :3001 / reco :8000 / DB / Redis / seed（§9〜§10） |
| S1 smoke | `/recommendations` 起点 → 実行 → 結果一覧。API health 未達時は skip |
| 環境変数 | `WEB_BASE_URL`（既定 `http://localhost:3000`）/ `API_BASE_URL`（既定 `http://localhost:3001`） |

証跡: `ai-logs/experiments/2026-07-17-lane-1e-playwright-d2.md`
...
| API-PUB-002 縦串 | §10.4〜§10.4.13 — レーン1e 親 Epic #1263 受け入れ充足（items≥1・itemPrice・alcohol NG）。Reason は後続 |
| NG正規化 api 正本 | §10.4.14 — レーン1f Epic #1320（api 派生 ngKeywords + reco 受信） |
| 手動 E2E（D1） | §10.4.15 / [レーン1e_手動E2Eチェックリスト.md](./レーン1e_手動E2Eチェックリスト.md) — Epic #1330。S1〜S6 pass（S2 再検証含む） |
| SCR-005〜007 回帰 | §10.4.16 — Epic #1418。S5' / S7 / S8 pass |
| Reason Postgres | §10.4.10b — Epic #1417 |
...
| 2026-07-15 | §10.4.14 / §13 を更新。レーン1f NG正規化 api 正本化（Issue #1324） |
| 2026-07-15 | §10.4.15 / §13 を更新。レーン1e 手動 E2E D1 チェックリスト（Issue #1331） |
| 2026-07-16 | §10.4.15 を更新。S2 empty 経路の手動再検証を `pass` に反映（#1345 後） |
| 2026-07-17 | §10.4.10b を追加。Reason Postgres 永続（Issue #1413 / Epic #1417） |
| 2026-07-17 | §10.4.16 を追加。SCR-005〜007 回帰 E2E シナリオ定義（Issue #1419）。当初 Docker 未起動で blocked |
| 2026-07-17 | §10.4.16 を更新。S5' / S7 / S8 手動実行を `pass` に反映（Issue #1419） |
| 2026-07-17 | §10.4.15 後続と §10.4.16 を整理。PR #1425 の develop merge コンフリクトを解消 |
