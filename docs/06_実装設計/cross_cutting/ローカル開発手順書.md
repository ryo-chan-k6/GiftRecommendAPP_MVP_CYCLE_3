...
| 5 | test-data | item / feature / meaning / embedding は縦串 item 側として充足。`recommendation_request` seed 追加だけでは No.3〜4 を解消しない | 本 Task では **seed 拡充せず**、不足は後続 Task 明示で止める |
...
**#1139:** `DATABASE_URL` 経由で `recommendation_request` を実永続化。商品返却までの残ギャップは No.3〜4。
---
...
| PUB-002 が 404 | Router 未マウントの場合は `createApp()` を確認（#1138 で配線済み）。残る 4xx/5xx は §10.4.4 No.3〜4 |
