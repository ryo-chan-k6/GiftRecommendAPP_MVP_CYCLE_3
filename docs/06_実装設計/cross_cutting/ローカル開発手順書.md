...
- 各アプリの起動順序と疎通確認（§9 実行手順。health 成功は Phase4 以降）
...
Phase4 実装前（§9.4 placeholder）では dev プロセスが即終了するため、通常は停止操作不要。

### 9.4 placeholder / skip 条件（Phase4 実装前）

| 条件 | 挙動 |
| ---- | ---- |
| 各 app の `dev` script 未実装 | 現行 repo では **placeholder**（メッセージ出力後に即終了）。本 Task の acceptance では health 成功を必須としない |
| `.env` 未作成 | script は warn を出して起動を試行する。初回は `copy-env-example.sh` を実行 |
| pnpm 未インストール | script は error で終了 |
| batch 本格起動 | `pnpm dev:batch` は骨格のみ。ジョブ CLI・定期実行は Phase4b |

Phase4a/4b で各 app のエントリポイント・health endpoint が整備されたら、§10.3 の疎通確認を更新する。

---

## 10. 疎通確認

一括実行は `scripts/dev/smoke-check.sh` を使用する（段階チェック・`--skip-*` 対応）。Phase3b（placeholder）では **app health 成功を必須としない**。

### 10.0 一括 smoke check（推奨）

```bash
# Phase3b: env + DB + Redis のみ（app は placeholder のため --skip-apps 推奨）
./scripts/dev/smoke-check.sh --skip-apps

# Phase4 以降: health 成功も確認（reco / api / web 起動済みであること）
...
| `--skip-apps` | reco / api / web health を省略（Phase3b 推奨） |

app health は接続不可・非 200 の場合 **skip** とし、Phase3b では script 全体を fail させない（§9.4 placeholder 整合）。
...
### 10.3 アプリ health（PUB-001 / INT-001 実装前は optional）

現行（2026-07 時点）では §9.4 の placeholder のため、health 成功は **必須確認対象外**。`smoke-check.sh` は接続不可・非 200 を **skip** とし、script 全体を fail させない。

| 確認対象 | URL（OpenAPI 正本） | 現状（develop / Task #1133 検証） |
| -------- | ------------------- | --------------------------------- |
| reco health | `GET http://localhost:8000/internal/reco/v1/health`（[internal-reco-api.yaml](../../../packages/contracts/openapi/internal-reco-api.yaml) / API-INT-001） | **skip** — `dev:reco` は placeholder。FastAPI エントリは存在するが health route 未実装 |
| api health  | `GET http://localhost:3001/api/v1/health`（[public-api.yaml](../../../packages/contracts/openapi/public-api.yaml) / API-PUB-001） | **skip** — `dev:api` は placeholder |
| web トップ | `GET http://localhost:3000/` | **skip** — `dev:web` は placeholder |

API-PUB-002（レコメンド実行）の **domain / infrastructure コード**は develop 上に存在するが、上記 health endpoint の HTTP サーバ起動は PUB-001 / INT-001 Task 完了後に §10.0 の `--skip-apps` なし実行で確認する。
...
| `smoke-check.sh` が Redis で失敗 | Redis 未起動、`REDIS_URL` 不一致、`redis-cli` 未インストールかつ Redis コンテナ未起動 | §7.2 / `start-redis.sh`。`redis-cli` なしでも compose 起動済みなら smoke-check が docker 経由で PING する |
| app health が skip のみ | Phase3b placeholder または app 未起動。`--skip-apps` または Phase4 実装後に再確認 |
...
| apps 実装 | §9 実行手順を各 apps README からリンク。placeholder を実サーバ起動へ差し替え |
