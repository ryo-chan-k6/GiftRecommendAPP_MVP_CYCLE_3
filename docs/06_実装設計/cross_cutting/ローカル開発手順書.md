...
| S2 0件 | **fail** | 0件条件が `GRS-REC-012` 500 → SCR-009 未達（別 Issue 候補） |
| S3 エラー | **pass** | api 停止でエラー UI → 条件入力へ戻る |
| S4 再検索 | **pass** | 結果→SCR-002 |
| S5 / S6 | **pass** | 理由詳細 / alcohol NG |
| Residual | SCR-001 `/` client exception | HomePage（別 Issue 候補） |

後続: S2 empty 経路の Issue 化／SCR-001 `/` 例外調査／D2（Playwright）。
---
