...
Python **3.14** と [uv](https://docs.astral.sh/uv/) を使用する。`.venv` は **worktree ごと**にリポジトリルート（および `apps/reco`）へ作成され、Git 管理しない（`.gitignore` 済み）。
...
| 疎通確認 | `redis-cli -u "$REDIS_URL" PING`（期待: `PONG`） |
...
現行（Phase3b）では §9.4 の placeholder のため、health / 結合成功は **必須確認対象外**。実装完了後に以下を実行する。
...
| `smoke-check.sh` が Redis で失敗 | Redis 未起動、`REDIS_URL` 不一致。§7.2 / `start-redis.sh` |
