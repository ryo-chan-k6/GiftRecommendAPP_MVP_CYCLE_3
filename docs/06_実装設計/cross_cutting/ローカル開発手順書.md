...
| DB migration 詳細        | Phase2 Epic（`db-physical-design`）       |
...
| 項目              | 方針                                                                 |
| ----------------- | -------------------------------------------------------------------- |
| docker-compose    | **同梱しない**。PostgreSQL / Redis は手動起動または Neon 等クラウド dev |
...
| Node.js         | web / api                     | バージョン pin は後続 Task で確定         |
| pnpm            | monorepo 依存管理             | ルート `package.json` 整備後に利用        |
| Python          | reco / batch                  | バージョン pin は後続 Task で確定         |
...
| PostgreSQL     | `localhost:5432`（例）         | Neon またはローカル |
...
**docker-compose は提供しない**（Human 判断 2026-06-07）。以下いずれかで dev 用 DB / Cache を用意する。

### 7.1 PostgreSQL

| 方式           | 概要                                      | `.env` 例（ダミー形式）                                      |
| -------------- | ----------------------------------------- | ------------------------------------------------------------ |
| ローカル起動   | 手元の PostgreSQL を起動                  | `postgresql://user:password@localhost:5432/gift_reco_dev`    |
| Neon dev       | クラウド dev ブランチの接続文字列を利用   | Neon ダッシュボードの接続 URL（**実値は `.env` のみ**）      |

- prod DB 接続文字列をローカルで使わない（[環境設計書 §7.1](./環境設計書.md)）
- schema 適用・migration は Phase2 Epic の手順を正本とする
