...
- 各アプリの起動順序と疎通確認（§9 実行手順。api / reco health は local-dev follow-up #1135 で最小実装）
...
| api | **実装済み（最小）**（Express + `GET /api/v1/health`） | `./scripts/dev/start-api.sh` / `pnpm dev:api` で **http://localhost:3001** 待受。前提: `pnpm install` |
| reco | **実装済み（最小）**（uvicorn + `GET /internal/reco/v1/health`） | `./scripts/dev/start-reco.sh` / `pnpm dev:reco` で **http://localhost:8000** 待受。前提: `./scripts/dev/setup-python-reco.sh`、`.env`（`RECO_INTERNAL_API_KEY` / `DATABASE_URL`） |
...
api / reco の health は OpenAPI（API-PUB-001 / API-INT-001）準拠の **ローカル最小実装**（Issue #1135）。本格実装仕様書・縦串は各識別子 Epic の後続 Task。
...
| api / reco health | §9.4 / §10.3 — Issue #1135 で最小 HTTP + health 実装済み。本格実装仕様・縦串は PUB-001 / INT-001 / PUB-002 後続 |
