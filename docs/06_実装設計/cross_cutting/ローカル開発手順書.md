...
| PUB-002 が 500 `GRS-REC-006` かつ user_feature row count mismatch | UserFeature が InMemory のまま（§10.4.7）。Postgres 永続配線が後続 |
...
| API-PUB-002 縦串 | §10.4〜§10.4.7 — UserSemantic Postgres 配線後も UserFeature 未永続で GRS-REC-006（#1272） |
