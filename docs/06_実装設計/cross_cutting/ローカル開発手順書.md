...
#### 10.4.5 レーン1e 結果あり再確認（2026-07-15 / Issue #1264）

目的: A2（縦串再確認）+ A3（items≥1）。SCR-001〜004 手動 E2E（D1）前提。

##### 実行サマリ（secret 非記載）

| 確認 | 結果 | 備考 |
| ---- | ---- | ---- |
| `check-env-names.sh --strict` | OK | worktree に `.env` 配置（MAIN から複製。値は非公開） |
| `seed-test-data.sh` | OK | items=3, item_feature=48 |
| `pnpm install`（worktree） | 要実施 | node_modules 欠如時 api は `tsc: not found` |
| api health | HTTP **200** | |
| reco health | HTTP **200** | `X-Internal-Api-Key` 付与 |
| `POST /api/v1/recommendations`（PUB-002） | HTTP **500** `GRS-REC-004` | ユーザー向けメッセージは「レコメンド処理に失敗…」 |
| DB `recommendation_request` / `recommendation_run` | **INSERT 済み**（`run_status=running`） | api→reco 呼び出しまで到達 |
| `error_log`（reco） | `recommendation_run not found: <run_id>` | `error_detail_json.phase_name=semantic_extracted` / `source_module_id=MOD-RECO-004` |
| items≥1 | **未達** | seed 不足が主因ではない |

##### 原因（事実）

- `recommendation_run` 行は Postgres に存在する（同一 `DATABASE_URL`）。
- MOD-RECO-004（User Semantic Extractor）は `run_validation.get_semantic_config_version_id(run_id)` で検証する。
- production composition（`build_production_ports`）は observability / config_resolver を Postgres 化するが、**user_semantic_extractor 側の `RunValidation` は `InMemoryRunValidation`（空）のまま**（`build_default_stub_ports` 由来）。
- そのため DB に run があっても「run not found」となり GRS-REC-004 で失敗する。

##### 本 Epic（#1263）での扱い

- `apps/reco` 配線修正は **epic_scope.allowed_paths 外**のため、本 Task ではコード修正しない。
- 手順・Blocker・後続 Issue を本節に残すことが完了条件。

##### 後続候補（Human 判断）

| 案 | 内容 | 推奨 |
| -- | ---- | ---- |
| A | reco composition で MOD-RECO-004（および同様の InMemory RunValidation 利用モジュール）に Postgres RunValidation を配線する専用 Epic/Task | **推奨**（結果あり縦串の本線） |
| B | D1 手動 E2E を sessionStorage / fixture 投入で暫定実施（SCR-004 結果画面のみ） | 画面 UX 確認には可。PUB-002 本線未検証のまま |
| C | OpenAI / seed 増強で様子見 | **非推奨**（今回の Blocker は seed/LLM ではない） |

関連 Issue: #1263 / #1264。experiments メモ: `ai-logs/experiments/2026-07-15-lane-1e-local-result-vertical.md`
---
...
| PUB-002 が 500 `GRS-REC-004` かつ error_log が `recommendation_run not found` | DB には run があるのに MOD-RECO-004 の InMemory RunValidation が空の可能性（§10.4.5）。seed / OpenAI 増強では解消しない |
...
| API-PUB-002 縦串 | §10.4 — Issue #1137〜。HTTP 200 商品返却は §10.4.5（#1264）時点で MOD-RECO-004 InMemory RunValidation により未達 |
| レーン1e 結果あり | §10.4.5 — Issue #1264。後続は Postgres RunValidation 配線（Human 判断） |
...
| 2026-07-15 | §10.4.5 / §11 / §13 を追加・更新。レーン1e 結果あり再確認。PUB-002 500 原因を MOD-RECO-004 InMemory RunValidation と記録（Issue #1264） |
