...
- 推薦理由詳細（SCR-005）を Item 単位のアコーディオン（画面内展開）として本画面内で扱えること
...
| 遷移元          | SCR-003（推薦成功・結果あり）。商品詳細（SCR-006）からの戻り、Feedback 表示の閉じる |
| 遷移先          | SCR-006（商品詳細）、SCR-005（理由詳細・同一画面内アコーディオン）、SCR-007（Feedback・導線）、SCR-002（再検索）、外部EC |
| 認証要否        | `false`（MVP 非認証）                                                |
| 権限条件        | なし                                                                 |
...
flowchart TD
  ...
  C -->|結果一覧へ戻る| B
  B -->|推薦理由詳細トグル| D[SCR-005 理由詳細・アコーディオン]
  D -->|折りたたみ| B
...
|   2 | 商品詳細を見る     | SCR-006（スタブ URL）          | Item カード操作              | MVP はスタブへ遷移。本体完成は別 Epic。未リンクにはしない |
|   3 | 推薦理由詳細を見る | SCR-005（アコーディオン展開）  | Item 単位                    | 独立 route にしない。複数 Item 同時展開可（排他なし） |
|   4 | Feedback入力       | SCR-007（導線・disabled）      | Item 単位                    | API-PUB-004 本実装は別レーン。`disabled` ＋短い説明 |
...
単一カラムの一覧画面。上部に画面タイトルと再検索導線、中央に順位付き商品カード相当の行（既存デザインルールに従い過剰なカード装飾は避ける）、各行に推薦理由要約・バッジ・理由詳細アコーディオン・操作ボタン群を配置する。

### 7.2 ワイヤーフレーム簡易図

SCR-005（推薦理由詳細）はモーダルではなく、各 Item 内のアコーディオンで表示する。複数 Item を同時に展開してよい（排他制御なし）。展開領域には高さ上限を設け、超過分は**詳細枠内スクロール**とする（一覧ページ全体のスクロールとは別）。
...
| Main   | Alert・結果一覧・理由詳細アコーディオン（同一画面内） | 結果あり時に一覧表示 |
| Footer | なし                                         | -                        |

### 7.4 SCR-005（アコーディオン）方針

| 項目 | 方針 |
| ---- | ---- |
| UI 形態 | Item 内アコーディオン（画面内展開）。独立 route・モーダルは採用しない |
| 初期状態 | 全 Item 折りたたみ |
| 同時展開 | 複数 Item 同時オープン可（排他なし）。比較しやすさと操作削減を優先 |
| トグル | `▶ 理由の詳細` で展開、`▼ 理由の詳細` で折りたたみ |
| 展開内容の優先順位 | `reason_points`（2〜3）→ 短い `reason_detail` → 必要時のみ `caution_note`。カード上の `reasonSummary` / badges の再掲は最小限 |
| 展開高さ上限 | 展開領域に高さ上限を設ける。超過分は詳細枠内スクロール（ページ全体スクロールは維持）。初期目安はモバイル優先で約 `12rem`（5〜7 行相当）。端末・実装時に微調整してよい |
| 詳細文が薄い場合 | Public に長文 detail が無いときは、展開領域に `reasonSummary` / badges を再掲し、「詳細な説明文はありません」相当の短い案内を出して折りたたみ可能とする |
...
|   9 | 推薦理由要約         | `reasonSummary`              | string  | テキスト   | △    | 値があるとき             | `isFallback=true` 時は汎用理由である旨を控えめに補足する（§11.5） |
|  10 | 推薦理由詳細トグル   | `reasonDetailToggle`         | action  | Button     | -    | Item 表示時              | SCR-005 アコーディオンを開閉。展開中は詳細領域を表示 |
|  11 | 商品詳細ボタン       | `itemDetailButton`           | action  | Button/Link| -    | Item 表示時              | SCR-006 へ。MVP はスタブ URL 導線とする（§10） |
|  12 | 外部EC遷移ボタン     | `externalEcButton`           | action  | Button/Link| -    | `itemUrl` があるとき     | 外部タブ |
|  13 | Feedback入力導線     | `feedbackEntry`              | action  | Button     | -    | Item 表示時（MVP）       | 本実装は別レーン。`disabled` ＋短い説明を表示（§11.4） |
...
|   2 | 推薦理由詳細を展開 | トグル（折りたたみ時） | 当該 Item の SCR-005 アコーディオンを展開 | 詳細表示 | - | 他 Item の展開状態は変更しない。長文 detail が無い場合は `reasonSummary` / badges 再掲＋短い案内（§7.4） |
|   3 | 推薦理由詳細を折りたたみ | トグル（展開時） | 当該 Item の SCR-005 を折りたたみ | 詳細非表示 | - | モーダル閉じる操作は不要 |
|   4 | 商品詳細を見る     | ボタン             | SCR-006 へ遷移（`itemId` 等） | 詳細画面またはスタブ | - | MVP はスタブ URL（プレースホルダ画面）へ遷移する。未リンク（導線非表示）にはしない |
|   5 | 外部ECで見る       | ボタン             | `itemUrl` を新規タブで開く | 外部サイト | URL 無しは非表示／無効 | |
|   6 | Feedback入力       | ボタン             | SCR-007 導線。本実装は別レーン | `disabled` のまま短い説明を表示 | - | API-PUB-004 呼び出しは本 Epic 実装段階では行わない。非表示にはしない |
|   7 | 再検索             | ボタン             | SCR-002 `/recommendations` へ。クエリ／sessionStorage 復元は SCR-002 §6.2 | 条件入力 | - | |
|   8 | 理由詳細枠内スクロール | 展開領域内操作 | 高さ上限超過時に詳細枠内のみスクロール | 続きを閲覧 | - | 一覧のページスクロールとは独立 |
...
| Feedback 未実装 | 導線を `disabled` ＋短い説明（例:「準備中」）で表示する。非表示にしない |
...
- `isFallback === true` の Item は、汎用理由である旨を控えめに補足する（強調ラベル・警告色・長い説明は使わない）
...
| ユーザー操作時 | アコーディオン開閉・外部遷移・再検索 | なし／導線 | |
...
| アクセシビリティ | トグルに明確な名前（展開／折りたたみ状態が分かる name / `aria-expanded`）。Feedback 等のモーダル利用時のみ focus trap / Esc 閉じるを推奨 |
...
| 操作ログ | 外部ECクリック・再検索・理由詳細展開／折りたたみ | 各操作 | MVP は client 計測最小で可 |
| エラーログ | 結果欠落 | 検知時 | |
...
- 再検索（SCR-002）
- 外部EC遷移（`itemUrl`）
- 推薦理由詳細（SCR-005）を Item 内アコーディオンとして本画面内で扱うこと（複数同時展開可・展開高さ上限・詳細枠内スクロール。詳細文が薄い場合は要約再掲で可）
- 商品詳細導線（MVP はスタブ URL）と Feedback 導線（`disabled` ＋短い説明）
...
|   7 | 理由詳細 | アコーディオンが開閉でき、複数 Item 同時展開できる | manual |
|   8 | 理由詳細高さ | 展開内容が上限超過時に詳細枠内スクロールになる | manual |
|   9 | レスポンシブ | 狭幅で操作可能 | manual |
...
- SCR-005 を独立画面・モーダルにしていないか（アコーディオン方針と一致するか）
- 複数同時展開・展開高さ上限・枠内スクロールが仕様どおりか
...
|  No | 論点 | 確定内容 | 参照 |
| --: | ---- | -------- | ---- |
|   1 | SCR-006 導線の到達先 | MVP はスタブ URL へ遷移する。未リンク（導線非表示）にはしない | §8・§10 |
|   2 | Feedback 導線の UI | `disabled` ＋短い説明を表示する。非表示にはしない。API-PUB-004 呼び出しは本 Epic では行わない | §8・§10・§11.4 |
|   3 | 詳細 API フィールドが要約のみの場合 | 展開領域に `reasonSummary` / badges を再掲し、詳細文が無い旨の短い案内を出す。折りたたみ可能とする | §7.4・§10 |
|   4 | `isFallback` の表示強度 | 控えめな補足のみ。強調ラベル・警告色・長い説明は使わない | §8・§11.5 |
|   5 | SCR-005 展開高さ上限の具体値 | 初期目安はモバイル優先で約 `12rem`（5〜7 行相当）。超過分は詳細枠内スクロール。実装時の微調整は可 | §7.4 |
