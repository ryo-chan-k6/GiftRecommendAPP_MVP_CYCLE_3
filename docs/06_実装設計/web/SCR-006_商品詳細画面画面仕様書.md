...
| 非表示項目 | Public Response に存在しない `reason_detail` / `reason_points` は **表示しない**（画面一覧 §4.6 の △ 項目。Public 契約上 sessionStorage にも無い） |
...
| `reason_detail` / `reason_points` | Public PUB-002 Response に含まれず、sessionStorage にも無い。 invent しない |
...
- `reason_detail` / `reason_points` の表示（Public Response に無いため）
...
|   4 | `reason_detail` / `reason_points` | Public Response に無いため MVP では表示しない | §8.1 |
