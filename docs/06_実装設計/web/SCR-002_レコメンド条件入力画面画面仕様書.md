...
| 項目            | 内容                                                                 |
| --------------- | -------------------------------------------------------------------- |
| 表示URL / Route | `/recommendations`（画面一覧 §4.2 を正とする。遷移図の `/recommend` との差分は §22） |
...
|   2 | 再検索 / 条件変更  | SCR-002             | SCR-004 / SCR-008 / SCR-009  | 前回入力の復元方針は §22     |
...
Semantic Config / Feature Rule（API-PUB-007 / 008）は初期取得対象とするが、MVP では画面上の追加入力 UI を必須としない（§22）。
...
|   4 | 予算上限       | `budgetMax`        | number | NumberInput| ○    | 空     | 整数・0以上                                   | 画面一覧は必須。API 契約上は任意（UI では必須） |
...
`freeText`（自由記述）は API 契約上任意。MVP 画面では非表示とし、必要になった場合は別途 Human 判断で追加する（§22）。
...
|   3 | API-PUB-007 Semantic設定取得       | `GET`  | `/api/v1/masters/semantic-configs` | 初期表示（並列） | 設定スナップショット（UI 露出は §22） |
|   4 | API-PUB-008 Featureルール取得      | `GET`  | `/api/v1/masters/feature-rules` | 初期表示（並列） | Feature Rule（UI 露出は §22） |
...
| budgetMax          | `budget.budgetMax`                          | UI必須。送信時は整数             | |
...
| 再表示時       | マスタ再取得またはキャッシュ | PUB-005〜008                | 入力復元方針は §22 |
...
- マスタ取得（少なくとも PUB-005 / 006。007 / 008 は取得のみでも可）
...
- Semantic / Feature Rule の詳細編集 UI
- 決済・購入（外部 EC は結果画面以降）
...
| --: | ---- | ---------------- | ------ | ---- | ---- |
|   1 | URL を `/recommendations` と `/recommend` のどちらに確定するか | 画面一覧と画面遷移図で不一致 | Human | 実装着手前 | 本仕様は画面一覧を暫定正とする |
|   2 | 再検索時の入力値復元方針 | SCR-004/008/009 からの戻りで UX が大きく変わる | Human | 実装着手前 | sessionStorage / クエリ / 未復元 |
|   3 | API-PUB-007 / 008 の画面上の見せ方 | 取得は必要だが入力 UI の要否が未確定 | Human | 実装着手前 | MVP は取得のみでも可と記載 |
|   4 | `freeText` 入力欄の追加要否 | API 契約にはあるが画面一覧に明示なし | Human | 実装着手前 | MVP 非表示を暫定 |
|   5 | `budgetMax` UI必須と API 任意の差分を維持するか | 画面一覧と契約仕様で差がある | Human | 実装着手前 | 本仕様は UI 必須を採用 |
