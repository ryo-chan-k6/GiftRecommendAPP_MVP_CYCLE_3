...
| 1 | `definition-update`（高優先）— 識別子 Epic `epic_scope` | `[Task]directory-structure-review:識別子Epic epic_scope・generated 整合`（#354） | **完了**（PR #355 merge） |
| 2 | `docs-update`（中優先）— 運用設計書・AGENTS・rules | `[Task]directory-structure-review:AGENTS・運用docs・rules のパス整合`（#352） | **完了**（PR #353 merge） |
| 3 | `migration-guide`（低優先）— 参照メモの整理 | Phase1 横断（Human 判断 2026-06-03） | **Phase0 対象外**（No.6–7・`reco-directory-architecture-redesign` Epic 旧表記含む） |
