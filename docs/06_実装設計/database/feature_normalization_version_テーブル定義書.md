...
| 観点 | `semantic_config_version` | `feature_normalization_version` |
| ---- | ------------------------- | ------------------------------- |
| 管理対象 | 意味推定ロジック・Feature ルール・Concept 定義 | Feature raw 値の正規化パラメータ（sigmoid center / k 等） |
...
| Featureルール定義書 | §14.8 は正規化パラメータを semantic_config_version 配下と記載 | 論理ER §11.1 / 物理ER / テーブル一覧 §9 は独立テーブルを定義 |

> 両者の責務分担の最終判断は §17 No.1（Human Review 必須）。
...
> **用語補足**: `model_version` / `ranking_config` は `created_at` を採用するが、論理ER §11.1 は本テーブルに `generated_at` のみ定義する。採用理由の最終判断は §17 No.3。
...
> **将来拡張キー（MVP では未使用）**: `z-score + sigmoid` 拡張用の `mu_feature` / `sigma_feature`（Feature 軸別）は §17 No.4 へ委譲。DDL CHECK への追加は後続 Task で判断する。
...
| 参照元 | 参照列 | 関係 | FK制約 | 備考 |
| ------ | ------ | ---- | ------ | ---- |
| `user_feature` | `feature_normalization_version_id` | normalizes | `LOGICAL` | 物理ER §9。reco が Run 実行時に解決・記録 |
| `item_feature` | `feature_normalization_version_id` | normalizes | `LOGICAL` | 物理ER §9・§11。batch が生成時に解決・記録。冪等キーに含む（テーブル一覧 §7） |

> MVP 初期 DDL では物理 FK を張らない。整合は batch / reco 側 version 解決 + seed 正本 + 派生行 INSERT 時の存在確認で担保する。参照元側 Index は各テーブル定義 Task で確定する。
...
| `uq_feature_norm_version_current_per_method` | UNIQUE（部分） | `normalization_method` | `is_current = true` は normalization_method あたり 1 行 | MVP 方針。§17 No.2 |
| `chk_normalization_method_mvp` | CHECK | `normalization_method` | `normalization_method IN ('sigmoid')` | MVP 1 値。`z_score_sigmoid` 等は §17 No.4 |
...
| ------ | ----------- | ------ | ------ | ---- |
| `normalization_method` | `normalization_method` | Featureルール定義書 §14.2（本 Task で候補値） | `sigmoid` | 将来 `z_score_sigmoid` 等は §17 No.4。enum Task 正本化は後続 |
...
| 7 | 冪等キー | item_feature の unique（物理ER §11 `uq_item_feature_idempotent`）に `feature_normalization_version_id` が含まれる | integration |
| 8 | seed 整合 | Featureルール定義書 §14.3 初期値と seed の `parameter_json` が一致 | manual |
...
| --: | ---- | ---------------- | ------ | ---- | ---- |
| 1 | `semantic_config_version` との責務分担 | Featureルール定義書 §14.8 は正規化パラメータを semantic_config_version 配下と記載。論理ER §11.1 は独立 `feature_normalization_version` を定義 | Human | DDL Task 前 | 推奨: 物理設計どおり独立テーブルを正とし、Featureルール定義書 §14.8 の整合は別 docs Task で検討 |
| 2 | `is_current` の解決単位 | MVP では `normalization_method` 単位 partial unique を採用。全体 1 現行のみも選択肢 | Human | seed Task 前 | model_version（model_type 単位）/ ranking_config（config_name 単位）パターンの踏襲 |
| 3 | `generated_at` vs `created_at` | 論理ER §11.1 は `generated_at`。他 Config テーブルは `created_at` | Human | DDL Task 前 | 本定義書は論理ER に従い `generated_at` を採用 |
| 4 | Feature 軸別パラメータ要否 | MVP は全 8 軸共通固定パラメータ（§14.3）。将来 z-score 拡張では軸別 μ / σ が必要 | Human | seed Task 前 | `parameter_json` に `per_feature` オブジェクト追加は後続。`normalization_rule` テーブルとの関係も要確認 |
| 5 | `z_score_sigmoid` 拡張時の `parameter_json` キー | Featureルール定義書 §14.7 の μ / σ を JSON に含めるか、統計量テーブルへ分離するか | Human | 将来 Task | MVP では `sigmoid` + center / k のみ |
...
| Featureルール | `docs/04_ドメインモデル設計/Featureルール定義書.md` | §14 sigmoid 正規化・初期パラメータ |
...
- `user_feature` / `item_feature` への LOGICAL FK 方針が明記されている
- item_feature 冪等キー（テーブル一覧 §7）への `feature_normalization_version_id` 含有が明記されている
- Featureルール定義書 §14.2 / §14.3 の sigmoid 初期パラメータが `parameter_json` 参照として明記されている
- `semantic_config_version` との責務分担が §5.1 / §17 で明示されている
