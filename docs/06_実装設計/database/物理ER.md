...
論理ER・テーブル一覧・正本定義表を入力とし、MVPで永続化する **61 物理テーブル**（テーブル一覧のうち `external_attribute` / `staging_attribute` を除く。Issue #843 で `lambda_ctx_rule` を追加）の関係、物理設計方針、制約・Index方針、後続テーブル定義書・DDLへの引き継ぎ事項を定義する。
...
- テーブル一覧で定義した MVP 作成対象 61 テーブルの PK / FK / 多重度 / 正本区分を物理設計レベルで整理する
...
| 対象テーブル群   | テーブル一覧 §13（MVP 作成対象 61。`lambda_ctx_rule` 含む）                 |
...
| 5 | `feature_rule`（論理ER上の抽象名） | `relationship_rule` / `occasion_rule` / `pair_rule` / `lambda_ctx_rule` / `concept_feature_rule` / `input_type_rule` / `feature_integration_rule` / `normalization_rule` へ **分解** | テーブル一覧 §8。`lambda_ctx_rule` は Issue #843 案A |
...
以下は主要テーブル群と関係の概観である。MVP 作成対象 61 テーブルの詳細は §8・§9 を正とする。
...
erDiagram
    ...
    semantic_config_version ||--o{ lambda_ctx_rule : "contains"
...
| `lambda_ctx_rule` | Lambda Context Rule | Semantic / Feature定義系 | 設定正本 | database / reco | `yes` |
...
| `evaluation_run` | `idx_evaluation_run_dataset_id` | `evaluation_dataset_id` | btree | 親 FK 補助・Dataset 単位履歴 | `evaluation_dataset_テーブル定義書` §5.4・`evaluation_run_テーブル定義書` §9 |
...
| Evaluation系（`evaluation_dataset` 等） | **365 日**（dataset 確定） | 原則自動 DELETE なし | `created_at` 基準（将来パージ） | `evaluation_dataset_テーブル定義書` §13・§17.1 No.5。Batch Log 90 日とは別枠 |
...
- MVP 作成対象 **61 テーブル**について `{物理テーブル名}_テーブル定義書.md` を 1 テーブル 1 Task で作成する（テーブル一覧 §1.1）
...
### 17.7 Human Review 決定事項（Issue #565 / `evaluation_dataset`）

| No | 論点 | 決定内容 | 備考 |
| --: | ---- | -------- | ---- |
| 1 | UNIQUE キー | **`dataset_name` + `dataset_version` 組み合わせ UNIQUE** | §10 `uq_evaluation_dataset_name_version` |
| 2 | `dataset_version` 形式 | **semver `v1.0.0`**（`chk_dataset_version_format`） | `semantic_config_version` 同型 |
...
| No | 論点 | 決定内容 | 備考 |
| --: | ---- | -------- | ---- |
| 1 | `recommendation_run_id` 物理列 | **MVP は物理列なし** | `evaluation_result.recommendation_result_id` 経由で間接連携 |
| 2 | `batch_run_id` 物理列 | **nullable 採用**（LOGICAL FK） | §10 `idx_evaluation_run_batch_run_id` |
| 3 | `evaluation_dataset_id` の物理 FK | **物理 FK ON** / `ON DELETE RESTRICT` | #565 §17.1 No.4 踏襲。§9 executed_by |
| 4 | version 列の物理 FK | **LOGICAL FK 維持**（物理 FK なし） | semantic_config_version §17.1 No.8 踏襲。§10 version Index 3 本 |
| 5 | `evaluation_run_phase_name` enum | **本 Task では enum 未定義** | phase_log DB CHECK 省略継続 |
| 6 | Evaluation Run 再実行方針 | **新規 Run INSERT**（同一 Run 再開なし） | 状態遷移設計書 §11.3 |
| 7 | Retention | **365 日**（`created_at`）。MVP 自動 DELETE なし | evaluation_dataset と同値 |
...
- MVP 作成対象 61 テーブルが明示され、`external_attribute` / `staging_attribute` が MVP 対象外であること
