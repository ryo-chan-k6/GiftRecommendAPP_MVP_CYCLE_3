...
Featureルール定義書 §2.1 / §11 の Input Type Rule を物理化する候補テーブルだが、物理ER・テーブル一覧では **MVP `partial`（△）** とし、MVP 初期では **コード定数 / YAML + seed** で代替可能とする（§17.1 No.1）。**Public API では返却しない**（API-PUB-007 / API-PUB-008 非公開）。
...
- reco が User Feature 生成時に、入力ブロック種別に応じた Rule ディスパッチを参照できるようにする
- MVP partial として、物理 DDL 作成要否を Human Review で判断できる粒度まで設計を確定する
...
| Feature 統合 | `feature_integration_rule` | `participates_in_feature_integration=true` の入力のみ統合対象（partial・#478） |
...
> 入力種別ディスパッチは Reco 内部完結。`application_method` / `default_source_weight` は Public 表面に露出しない。
...
| Featureルール §17.6 | YAML / JSON + seed 推奨 | MVP partial として DB テーブル化は **任意**（§17.1 No.1） |
...
| 7 | `default_source_weight` | Default Source Weight | `numeric(4,3)` | `no` | — | — | — | `NULL` | 統合時の既定重み（0.0〜1.0）。MVP では NULL 許容（reco 定数フォールバック可） |
| 8 | `display_order` | Display Order | `integer` | `yes` | — | — | — | `1` | ディスパッチ評価順（小さいほど先）。運用・デバッグ用 |
| 9 | `is_active` | Active Flag | `boolean` | `yes` | — | — | — | `true` | 有効フラグ。`false` は reco 参照対象外 |
...
| `chk_input_type_mvp` | CHECK | `input_type` | §11.1 の 7 値 IN 句 | enum定義書未整備。§17.1 No.3 |
| `chk_application_method_mvp` | CHECK | `application_method` | §5.1 の 6 値 IN 句 | ディスパッチ先コード |
| `chk_default_source_weight_range` | CHECK | `default_source_weight` | `NULL` または 0.0〜1.0 | 統合重み |
| `chk_invert_delta_non_preferred` | CHECK | `invert_delta`, `application_method` | `application_method='concept_feature_delta_invert'` のとき `invert_delta=true` を推奨（違反時は WARNING 相当。MVP は CHECK 緩和可） | §17.1 No.4 |
...
| ------ | ----------- | ------ | ------ | ---- |
| `input_type` | `input_type` | Featureルール定義書 §11.1 | 7 分類（上記 CHECK） | enum定義書への正本化は §17.1 No.3 |
| `application_method` | `application_method` | 本定義書 §5.1 | 6 ディスパッチコード | reco 実装の分岐キー |
| `invert_delta` | — | Featureルール §11.3 | `true` / `false` | non_preferred 専用 |
| `participates_in_feature_integration` | — | Featureルール §11.5 / §12 | `true` / `false` | Hard Filter 系は `false` |
| `default_source_weight` | — | concept_feature_rule §5.3 参照 | 0.0〜1.0 または NULL | `source_weight` の DB 正本化は partial |
...
| SELECT | reco | 該当 `input_type` 行なし | — | — | reco 定数フォールバック（§11.1 既定経路）を使用 |
| INSERT | database（seed） | 新 version 初回投入 | 全列 | version ごと Upsert | MVP は 7 行固定 seed を想定（§17.1 No.2） |
| UPDATE | database（運用） | ディスパッチ調整・無効化 | `application_method`, `invert_delta`, `default_source_weight`, `is_active` | — | **`input_type` 変更禁止** |
...
| MVP 代替 | 物理テーブル未作成時は reco 定数 / YAML が正本（§17.6） |
...
| 適用順序 | 物理ER §15: `semantic_config_version` 作成後、Rule 群の一部として適用。**MVP partial のため DDL Task で作成省略可**（§17.1 No.1） |
...
| No | 観点 | 確認内容 | 種別 |
| --: | ---- | -------- | ---- |
| 1 | DDL適用 | CREATE TABLE / Index / FK / CHECK（作成する場合） | migration |
...
| 7 | 定数フォールバック | 物理テーブル未作成時に §11.1 既定経路で動作する | manual |
...
| --: | ---- | ---------------- | ------ | ---- | ---- |
| 1 | MVP で物理テーブルを作成するか | 物理ER partial / テーブル一覧 △。§17.6 は YAML 推奨 | Human | DDL Task 前 | §17.1 No.1 |
| 2 | MVP seed 行数（7 行固定か） | `ng_condition` / `budget_condition` を DB 行として保持するか | Human | seed Task 前 | Hard Filter 分離方針と連動 |
| 3 | `input_type` enum 正本化 | enum定義書に §11.1 7 分類を追加するタイミング | Human | enum 更新 Task | 現状は CHECK 候補値 |
| 4 | `default_source_weight` の MVP 採用 | `feature_integration_rule`（#478）との境界 | Human | integration Rule Task | NULL 許容で開始可 |
| 5 | `chk_invert_delta_non_preferred` の厳格度 | DDL で HARD CHECK にするか WARN 相当にするか | Human | DDL Task | §10 注記 |

### 17.1 Human Review 論点（MVP partial）

| No | 論点 | 推奨案（AI） | 判断者 | 備考 |
| --: | ---- | ------------ | ------ | ---- |
| 1 | MVP 物理 DDL 採用 | **初回 MVP ではテーブル作成を省略**し、reco 定数 + §11.1 を正本とする。version 管理が必要になった段階で本テーブルを DDL 化 | Human | Featureルール §17.6・物理ER partial と整合 |
| 2 | seed 方針 | テーブル作成時は **version あたり 7 行固定**（§11.1 全種別）を seed | Human | ng/budget もディスパッチ明示のため行保持を推奨 |
| 3 | Public API | **非公開**（API-PUB-007 / API-PUB-008） | Human | pair_rule と同型 |
| 4 | `semantic_config_version_id` FK | テーブル作成時は **物理 FK ON**、ON DELETE RESTRICT | Human | semantic_config_version §8.1 と同型 |
...
| enum定義書 | `docs/06_実装設計/database/enum定義書.md` | feature_code 等（input_type は未整備） |
...
- table-spec §17 に MVP partial 採用方針（物理 DDL 省略可）が未決事項として整理されている
- OpenAPI / generated 変更が含まれていない（#469 委譲）
- DDL Task が CREATE TABLE を起こせる粒度である（作成する場合）
