...
> MVP 初期 DDL では物理 FK を張らない。整合は reco / batch 側 Config 解決 + seed 正本 + Run INSERT 時の存在確認で担保する。`evaluation_run` 側 Index は `idx_evaluation_run_ranking_config`（`evaluation_run_テーブル定義書` §9・§17.1 No.4 確定）。
