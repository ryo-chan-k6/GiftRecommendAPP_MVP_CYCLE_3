...
| MVP対象        | `yes`                                              |
| 作成日         | 2026-06-16                                         |
| 更新日         | 2026-06-20（Layer2 ci-db.yml CI migration 検証 #672） |
...
対象は `supabase/migrations/`（適用正本）、`supabase/seeds/`（master seed SQL 正本）、`db/ddl/`（設計参照用 DDL）、および Supabase Storage（Raw JSON 保存）との連携規約である。
...
| 対象（関連） | Raw JSON 保存先としての Supabase Storage（**接続方針 A**: S3 互換 API + `OBJECT_STORAGE_*`。#1612 client） |
...
| `supabase/seeds/masters/` | master seed 投入 SQL | seed SQL 正本 | Phase3 A4 / ⑤ 移行 |
...
> `db/migrations/` は本プロジェクトでは **使用しない**。適用正本は `supabase/migrations/` とする（[実装フェーズ実行プロセス設計書](../../../00_共通/プロジェクト管理/実装フェーズ実行プロセス設計書.md) §6.3、[プロジェクトディレクトリ構成定義書](../../../00_共通/ディレクトリ構成/プロジェクトディレクトリ構成定義書.md) §9.1 と整合）。
...
- [x] CLI バージョン pin（Phase3 A3）
- [x] CI での migration 検証範囲（Layer2 `ci-db.yml` で `supabase start` + `migration up` + master seed。通常 PR CI `ci.yml` とは分離）
- [ ] 本番への適用主体（手動 workflow_dispatch / deploy パイプライン連携）

#### 6.4.1 Layer2 CI migration 検証（Epic C C1）

| 項目 | 内容 |
| ---- | ---- |
| workflow | [`.github/workflows/ci-db.yml`](../../../.github/workflows/ci-db.yml) |
| トリガー | `workflow_dispatch` / `workflow_call`（Layer2 テスト workflow から reusable 呼び出し） |
| 実行内容 | `supabase start` → `supabase migration up` → `supabase/seeds/masters/` 投入（`scripts/db/` 経由） |
| CLI pin | `supabase/.cli-version` |
| 分離方針 | 通常 PR 品質ゲート（`ci.yml` / `ci-contract.yml`）へ混在させない |

Task Branch 検証用に `chore/task-672-ci-db-ephemeral` への push（`supabase/**` / `scripts/db/**` / workflow 変更時）でも起動する。本番 Supabase プロジェクトへの link / `db push` は対象外とする。
...
```text
supabase/
├─ config.toml           # Supabase CLI 設定（[db.seed] 含む）
├─ migrations/           # ★ DB 適用正本
└─ seeds/
   ├─ masters/           # ★ master seed SQL 正本
   └─ test-data/         # ローカル / テスト用（db reset 自動投入外）
...
| `supabase/seeds/masters/` | テーブルまたは論理グループ単位。`config.toml` [db.seed] で glob 指定 |
| `supabase/seeds/test-data/` | ローカルのみ。本番適用対象外。自動 seed に含めない |
...
### 8.3 seed ファイル（`supabase/seeds/`）

| パス | 形式例 | 備考 |
| ---- | ------ | ---- |
| `masters/` | `{table_or_group}.sql` | `semantic_config.sql`, `relationship_master.sql` |
| `test-data/` | `{scenario}.sql` | 本番非適用。README に明記 |
| `config.toml` `[db.seed]` | 必須 | `sql_paths = ["./seeds/masters/*.sql"]`。`supabase/seed.sql` 単体正本は非採用 |

投入は Supabase CLI `db reset`（推奨）または `./scripts/db/seed-masters.sh`（`psql`）を用いる。手順は [DB構築手順書](./DB構築手順書.md) §9。
...
| バケット名（正本） | **`raw-products`**（全環境共通。環境分離は Supabase プロジェクト単位。`OBJECT_STORAGE_BUCKET` と一致） |
| 公開範囲 | **private**（`public = false`）。web / anon からの直アクセスを想定しない |
| 定義正本 | `supabase/migrations/20260725120000_raw_products_storage_bucket.sql`（#1616） |
| 実装 | batch の `S3CompatibleObjectStorageClient`（#1612）。接続は **案 A（S3 互換）**。公式 SDK / service_role 直結（案 B）は採用しない |
| 認証 | `OBJECT_STORAGE_ACCESS_KEY` / `OBJECT_STORAGE_SECRET_KEY`（DB 用 `SUPABASE_SERVICE_ROLE_KEY` とは分離） |
| Hosted 初回 | **migration 適用を正**とする。Dashboard 手動作成はフォールバック。S3 互換キー発行・Secrets 登録は **Human** |
| Storage RLS | MVP では `storage.objects` 向け Policy を追加しない（アクセスは S3 互換キー）。詳細は将来 Task |

[インターフェース一覧](../../../05_アプリケーション設計/アプリ/インターフェース一覧.md) §10 Storage IF は、物理サービス=Supabase Storage・方式=S3 互換 REST（接続方針 A）として記載する。

ローカル適用の概要は [ローカル開発手順書](../cross_cutting/ローカル開発手順書.md) §7.4、`supabase/` の入口は [supabase/README.md](../../../supabase/README.md)。
...
flowchart TD
  ...
  verify["スキーマ・制約の検証"]
  seed["supabase/seeds/masters 投入"]
  done["完了"]
...
| seed | `db reset` 時に `config.toml` [db.seed] 経由で `supabase/seeds/masters/` を自動投入 |
| test-data | ローカルのみ |
...
| MVP△ | `evaluation_*` / 各種 `*_metric` は Task scope で go/no-go。`external_attribute` / `staging_attribute` は **DDL 不含**（Human 判断・物理ER §17 No.7） |
...
| 5 | MVP△ テーブル含有 | **確定**: `external_attribute` / `staging_attribute` は DDL 不含（60 テーブル）。`evaluation_*` / `reco_score_distribution_metric` は DDL 含有のまま | — |
| 6 | CI migration apply | 将来扱い | Phase3 / Phase4 |
| 7 | ディレクトリ構成定義書の `db/migrations/` | **完了**: §9.1 で `supabase/migrations/` 正本・`db/migrations/` 不使用を明記。実装フェーズ設計書 §6.3 も整合 | — |
