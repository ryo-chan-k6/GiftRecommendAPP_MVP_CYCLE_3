...
UNION ALL SELECT 'lambda_ctx_rule', count(*) FROM lambda_ctx_rule;
```

期待値: 12, 15, 180, 96, 120, 112, **0**（`lambda_ctx_rule` は DDL 適用後・MVP 初期 seed 0 行。テーブル未作成時は当該 SELECT を除外）
