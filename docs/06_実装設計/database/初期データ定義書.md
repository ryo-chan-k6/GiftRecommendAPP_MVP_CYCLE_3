...
| 対象外 | `semantic_rule` の完全 seed（後続 Task） |
| 対象 | `concept_feature_rule` の **稀疏** seed（§10.3 仮説値。全 18×8 完全行列は必須としない） |
...
flowchart TD
  ...
  s08["08_pair_rule"]
  s08b["08b_lambda_ctx_rule"]
  s09["09_config_versions"]
  s10["10_feature_integration_rule"]
  s11["11_concept_feature_rule"]

  mig --> s01 --> s02 --> s03 --> s04 --> s05
  s05 --> s06 --> s07 --> s08 --> s08b --> s09 --> s10 --> s11
```

| 順 | ファイル | 対象テーブル | 行数（MVP） | 正本参照 |
| -- | -------- | ------------ | ----------- | -------- |
| — | `supabase/migrations/20260617120000_initial_schema.sql` | 全スキーマ | — | `db/ddl/d01..d12`（`lambda_ctx_rule` は後続 migration） |
...
| 8b | `08b_lambda_ctx_rule.sql`（後続 seed Task） | `lambda_ctx_rule` | **0**（初期） | `lambda_ctx_rule_テーブル定義書` §13 / Issue #843 |
| 9 | `09_config_versions.sql` | `model_version`, `ranking_config`, `feature_normalization_version` | 3, 1, 1 | 各テーブル定義書 §6.1 |
| 10 | `10_feature_integration_rule.sql` | `feature_integration_rule` | 48（8 Feature × 6 input source） | Featureルール §12.3 / `feature_integration_rule_テーブル定義書` §11.1 |
| 11 | `11_concept_feature_rule.sql` | `concept_feature_rule` | **112**（18 Concept × 8 軸のうち非ゼロのみ。稀疏） | Featureルール §10.3 / `concept_feature_rule_テーブル定義書` §17.1 |
...
### 8.7.1 Lambda Context Rule（MVP 初期 0 件）

| テーブル | 行数 | 値の正本 |
| -------- | ---- | -------- |
| `lambda_ctx_rule` | **0**（初期） | `lambda_ctx_rule_テーブル定義書` §13 / §17.1 |

- MVP 初期は **seed 行なし**。MOD-RECO-009 は Rule 未設定時 **0.5 フォールバック**で継続する
- 代表 relationship × occasion の初期値投入は後続 seed Task（Human 判断で件数・値を確定）
- seed SQL 実体（`08b_lambda_ctx_rule.sql`）は本 docs の方針正本のみ。ファイル作成は後続 Task
...
### 8.11 Feature Integration Rule（48 件）

固定 `semantic_config_version_id`（`a1111111-1111-4111-8111-111111111102`）に対し、MVP 8 Feature 軸 × 6 `input_source` の全組み合わせを投入する。

| input_source | weight | 行数 |
| ------------ | -----: | ---: |
| `relationship_feature` | 0.500 | 8 |
| `occasion_feature` | 0.500 | 8 |
| `pair_delta` | 1.000 | 8 |
| `preferred_delta` | 1.000 | 8 |
| `avoid_delta` | 1.000 | 8 |
| `free_text_delta` | 0.700 | 8 |

- 値の正本: [Featureルール定義書 §12.3](../../04_ドメインモデル設計/Featureルール定義書.md)
- 物理仕様: [feature_integration_rule_テーブル定義書 §11.1](./feature_integration_rule_テーブル定義書.md)
- 冪等キー: `semantic_config_version_id`, `feature_code`, `input_source`
- 再投入時は `weight` / `is_active` を正本値へ更新する

### 8.12 Concept Feature Rule（稀疏 112 件）

固定 `semantic_config_version_id`（`a1111111-1111-4111-8111-111111111102`）に対し、Featureルール定義書 §10.3 の signed delta を物理列へ分解して投入する。

| 物理列 | 変換 |
| ------ | ---- |
| `feature_delta` | `|signed_delta|`（0.0〜1.0） |
| `polarity` | signed > 0 → `positive` / signed < 0 → `negative` |
| 行の有無 | signed = 0.00 は **行を作らない**（稀疏。寄与 0） |

- 値の正本: [Featureルール定義書 §10.3](../../04_ドメインモデル設計/Featureルール定義書.md)
- 物理仕様: [concept_feature_rule_テーブル定義書](./concept_feature_rule_テーブル定義書.md) §6・§17.1
- `semantic_concept_id` は `semantic_concept.concept_code` から JOIN 解決（UUID ハードコードしない）
- 冪等キー: `semantic_config_version_id`, `semantic_concept_id`, `feature_code`
- 再投入時は `feature_delta` / `polarity` / `is_active` を正本値へ更新する
- 期待行数: **112**（18 Concept × 8 軸 = 144 のうち非ゼロのみ）
...
UNION ALL SELECT 'feature_integration_rule', count(*) FROM feature_integration_rule
UNION ALL SELECT 'concept_feature_rule', count(*) FROM concept_feature_rule;
```

期待値: 12, 15, 180, 96, 120, 112, **48**, **112**。

`lambda_ctx_rule` は後続 migration / seed Task の対象であり、テーブルが存在する環境だけ個別に確認する。

```sql
SELECT count(*) FROM lambda_ctx_rule;
```

期待値: **0**（MVP 初期）。テーブル未作成時は実行しない。
...
- `feature_integration_rule` は 3 列の冪等キーで `weight` / `is_active` を正本値へ収束させる
...
| 2026-07-30 | `feature_integration_rule` 48 行の投入順・初期値・検証を追加 | Issue #1757 |
| 2026-08-06 | `concept_feature_rule` 稀疏 112 行の投入順・初期値を追加 | Featureルール §10.3 |
