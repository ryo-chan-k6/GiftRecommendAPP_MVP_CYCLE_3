...
> `staging_item` は **`staging_item_テーブル定義書`**（#517）で定義済み。`staging_item_image` 等は別 Task。本定義書では **`raw_metadata_id` 被参照** を確定する。
...
| `staging_item_image` | `raw_metadata_id` | transforms_to | `LOGICAL` | Staging 定義 Task |
