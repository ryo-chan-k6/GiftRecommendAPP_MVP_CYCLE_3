...
model_version           = 順位の決め方（ドメイン概念上の表現）
ranking_config          = Rankingパラメータ正本（MVP DB 物理設計）
```

> MVP DB では Ranking パラメータ（重み・MMR・top_k 等）の正本は `ranking_config` とする。`model_version` は LLM / Embedding / Reason 生成など技術モデル識別の正本であり、Ranking パラメータは含まない。詳細は [ranking_config_テーブル定義書](../06_実装設計/database/ranking_config_テーブル定義書.md) §17.1 および [Ranking定義書](./Ranking定義書.md) §13 を参照する。ドメインモデル全体の全面更新は別 Issue とする。
