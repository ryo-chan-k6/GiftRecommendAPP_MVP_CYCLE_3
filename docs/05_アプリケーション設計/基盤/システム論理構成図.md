...
また、外部商品データ取得においては、DB容量節約と再処理可能性を両立するため、外部APIレスポンスのRawデータ本体はSupabase Storageに保存し、DBにはRawデータの参照・管理に必要なメタデータのみを保存する。
...
| Rawデータ本体はObject Storageに保存する | DB容量節約と再処理可能性のため、外部APIレスポンス原本はSupabase Storageに保存する     |
...
| 商品データ収集     | batchの責務                          |
| 外部商品API接続    | batchの責務                          |
| Rawデータ本体保存  | batchが Supabase Storage へ保存する      |
...
| Raw保存           | 外部APIレスポンスの生データをSupabase Storageへ保存する         |
| Rawメタデータ保存 | Storage object key、hash、取得日時、取得元API、取込状態をDBへ保存する |
| Staging変換       | Supabase Storage 上のRawデータを読み込み、内部形式へ変換する                    |
...
| `Raw Data Writer`           | RawデータをSupabase Storageへ保存                        |
| `Raw Metadata Writer`       | Rawデータのobject key、hash、取得日時、取込状態をDBへ保存 |
| `Staging Transformer`       | Supabase Storage 上のRawデータを読み込み、内部形式へ変換                 |
...
ただし、外部APIレスポンスの生データ本体はDBには保存せず、Supabase Storageに保存する。  
...
| 推薦スコア計算   | recoの責務                   |
| 外部API呼び出し  | batch等の責務                |
| Raw JSON本体保存 | Supabase Storageの責務      |
| UI表示           | webの責務                    |
...
| 対象          | 保存先            | 理由                                |
| ------------- | ----------------- | ----------------------------------- |
| Raw JSON本体  | Supabase Storage | 大容量・再処理用原本のため          |
...
| 主な利用元  | batch                                                           |
| Online利用  | MVPでは原則しない                                               |
| Raw保存方針 | 取得レスポンス本体はSupabase Storageへ保存する                 |
...
| batch      | object storage       | HTTPS / Supabase Storage API      | 外部APIレスポンスRawデータ保存・読取                                  |
...
| Raw本体保存       | batch / object storage | 外部レスポンス本体をSupabase Storageへ保存           |
...
| Raw本体は Supabase Storage に保存する             | DB容量を抑え、再処理可能性を維持する                   |
...
| Raw Metadata       | raw_product_object_key, content_hash, fetched_at, source_api, import_status | Supabase Storage 上のRawデータを参照・管理する |
...
| item                   | 正本             | アプリ内の商品正本                                              |
| raw_product_data       | 外部取得原本     | Supabase Storageに保存する。外部APIレスポンスの再処理用原本    |
| raw_product_metadata   | 管理メタデータ   | DBに保存する。Storage object key、hash、取得日時、取込状態を管理する |
...
| Supabase Storage権限      | batchからの書込・読取権限を最小限にする |
...
| DB配置             | Supabase（PostgreSQL + Storage）        |
| Object Storage配置 | Supabase Storage（MVP）。将来は他 S3 互換も検討可 |
...
| Raw保存方針        | Rawデータ本体がDBではなく Supabase Storage に保存されているか                    |
