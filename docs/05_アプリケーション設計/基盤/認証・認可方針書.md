...
| database        | Supabase / DB            | DB接続権限                | データ永続化                                   |
...
| API                                          | 匿名利用 | 主な認可制御                                          |
| -------------------------------------------- | -------- | ----------------------------------------------------- |
| `GET /masters/relationships`                 | ○        | 公開参照のみ                                          |
| `GET /masters/occasions`                     | ○        | 公開参照のみ                                          |
| `POST /recommendations`                      | ○        | 入力Validation、Rate Limit                            |
| `POST /recommendations/{result_id}/feedback` | ○        | 対象Result検証、重複抑制、Rate Limit                  |
| `GET /items/{item_id}`                       | △        | MVPで必要な場合のみ。公開表示情報に限定               |
| `GET /recommendations/{result_id}`           | △        | 初回MVPでは原則不要。導入する場合は推測困難ID等で制御 |
...
| ------------------------------------ | ---------------- | ---------- | -------------------------------------------- |
| `POST /internal/recommendations/run` | api              | reco       | Internal API Key必須                         |
| `GET /internal/health`               | api / monitoring | reco       | 内部ネットワークまたはKey制御                |
...
| API                                          | Validation観点                                               |
| -------------------------------------------- | ------------------------------------------------------------ |
| `POST /recommendations`                      | budget範囲、relationship、occasion、文字数、NG条件、必須項目 |
| `POST /recommendations/{result_id}/feedback` | result_id存在確認、対象item確認、rating範囲、comment文字数   |
| `GET /masters/relationships`                 | 原則なし                                                     |
| `GET /masters/occasions`                     | 原則なし                                                     |
...
| 対象                    | リスク                            |
| ----------------------- | --------------------------------- |
| `POST /recommendations` | 大量実行によるreco / LLM / DB負荷 |
| `POST /feedback`        | Feedbackスパム                    |
| Master取得API           | 過剰アクセス                      |
...
```text
- 推薦結果はPOST /recommendations のレスポンスで返す
- レコメンド履歴画面は作らない
- 過去結果をユーザー別に一覧表示しない
```
...
将来またはMVP内で `GET /recommendations/{result_id}` を実装する場合は、以下を検討する。
