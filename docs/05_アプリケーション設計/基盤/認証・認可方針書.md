...
| `OBJECT_STORAGE_ACCESS_KEY` | batch                      | ×       | Raw保存（Supabase Storage **S3 互換** Access Key。接続方針 A） |
| `OBJECT_STORAGE_SECRET_KEY` | batch                      | ×       | Raw保存（S3 互換 Secret Key）       |

**接続方針 A（確定 / #1614）:** Raw JSON の Object Storage 操作は `OBJECT_STORAGE_*`（S3 互換）を用いる。`SUPABASE_SERVICE_ROLE_KEY` は PostgreSQL / Supabase 管理用であり、Storage SDK 直結（案 B）の主経路にはしない。
