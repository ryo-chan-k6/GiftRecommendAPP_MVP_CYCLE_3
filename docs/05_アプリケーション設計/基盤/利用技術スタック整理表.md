...
本ドキュメントは、後続の `システム物理構成図`、`リポジトリ戦略設計書`、`API設計方針書`、`バッチ設計方針書`、`CI/CD方針書` の前提となる。
...
| リポジトリ戦略設計書     | monorepo構成・ディレクトリ設計の前提                   |
...
| DevOps方針書             | GitHub Actions、CI/CD、環境変数管理の前提              |
...
flowchart LR
    ...
    User --> Next
    Next -->|HTTPS| Express
    Express -->|Internal HTTP/HTTPS| FastAPI
    Express --> PG
...
| CSS            | Tailwind CSS等    | 候補     | UIスタイリング             |
| API通信        | fetch / axios     | 候補     | api呼び出し                |
...
| API仕様            | OpenAPI                  | 採用方針 | API仕様管理        |
| エラーハンドリング | 共通Error Middleware     | 採用方針 | APIエラー形式統一  |
| 内部通信           | HTTP/HTTPS Client        | 採用方針 | reco呼び出し       |
...
| Object Storage Client | boto3等                | 候補     | S3 / S3互換Storage操作   |
...
| ホスティング | Supabase / Neon等              | 候補     | PostgreSQLホスティング     |
...
| 項目           | 技術                      | 採用区分    | 用途                        |
| -------------- | ------------------------- | ----------- | --------------------------- |
| Object Storage | S3 / S3互換Object Storage | MVP採用方針 | Rawデータ保存               |
| API            | S3 API                    | 採用方針    | batchから保存・読取         |
...
| database       | Supabase / Neon等  | PostgreSQL hosting  | pgvector対応要確認        |
| object storage | S3 / S3互換Storage | Rawデータ保存       | DB容量節約                |
...
| batch          | object storage                      | HTTPS / S3 API          | Rawデータ保存・読取                                      |
...
| Object StorageはS3 APIとして扱う | 実体はHTTPS通信                                |
...
| Node.js         | Node.js           | web / api実行      |
| Python          | Python            | reco / batch実行   |
...
| ディレクトリ | 技術                 | 内容               |
| ------------ | -------------------- | ------------------ |
| `apps/web`   | Next.js / TypeScript | フロントエンド     |
...
| `S3_BUCKET_NAME`       | batch              | Raw保存先bucket               |
| `S3_ENDPOINT_URL`      | batch              | S3互換Storage利用時のendpoint |
| `S3_ACCESS_KEY_ID`     | batch              | Object Storage接続            |
| `S3_SECRET_ACCESS_KEY` | batch              | Object Storage接続            |
...
| Object Storage    | S3 / S3互換    | Raw JSON保存に適する                         |
...
| S3 API         | Rawデータ保存                 | object storage            |
...
| 後続設計接続       | 物理構成図、API設計、バッチ設計、DB設計へ展開できるか                      |
