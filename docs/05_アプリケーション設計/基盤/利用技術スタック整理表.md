...
| Object Storage Client | Supabase Storage SDK等 | 候補     | Raw JSON 保存・読取      |
...
| API            | Supabase Storage API      | 採用方針    | batchから保存・読取         |
...
| Object StorageはSupabase Storage APIとして扱う | 実体はHTTPS通信                                |
...
| 項目    | バージョン     | 正本ファイル                                      |
| ------- | -------------- | ------------------------------------------------- |
| Node.js | 24 LTS         | `.nvmrc` / `.node-version` / `package.json#engines` |
| Python  | 3.14           | `.python-version`                                 |

PR 品質ゲート（`.github/workflows/ci-contract.yml`）の Node.js 実行版は Task #653 で 24 LTS に統一する（開発正本との一時差分）。
...
| `SUPABASE_URL`              | batch              | Supabase プロジェクト URL（Storage 含む） |
| `SUPABASE_SERVICE_ROLE_KEY` | batch              | server 専用。Storage / DB アクセス        |
| `STORAGE_BUCKET_RAW`        | batch              | Raw JSON 保存バケット名（例: raw-products） |
...
| Supabase Storage API | Rawデータ保存                 | Supabase Storage                                                                      |
