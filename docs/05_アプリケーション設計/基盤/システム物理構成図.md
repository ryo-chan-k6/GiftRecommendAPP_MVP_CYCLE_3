...
| database             | Supabase Hosted PostgreSQL      | PostgreSQL + pgvector対応環境     |
| object storage       | Supabase Storage                | Raw商品データ保存                 |
...
MVPでは認証・認可の主境界は `api` に置き、Supabase Auth はアプリ接続しない（`service_role` をサーバ側用途に限定する）。
...
flowchart LR
    ...
    subgraph DBHOST[DB Hosting<br/>Supabase Hosted PostgreSQL]
        DB[(PostgreSQL)]
        PGV[(pgvector)]
    end

    subgraph OBJ[Object Storage<br/>Supabase Storage]
...
| database             | Supabase Hosted PostgreSQL  | PostgreSQL / pgvector          | アプリデータ、商品データ、Feature、Embedding、Result、Feedback |
| object storage       | Supabase Storage            | Supabase Storage API           | 外部商品APIレスポンスRaw JSON保存                              |
...
sequenceDiagram
    ...
    User->>Web: 条件入力・送信
    Web->>API: HTTPS POST /api/v1/recommendations
...
sequenceDiagram
    ...
    GHA->>Storage: HTTPS / Storage API Raw JSON保存
...
flowchart TD
    ...
    PRODUCT[External Product API<br/>Rakuten等]
    STORAGE[(Supabase Storage<br/>Raw Product JSON)]
    ...
    BATCH -->|HTTPS / Storage API<br/>Raw保存| STORAGE
...
| GitHub Actions batch | object storage       | HTTPS / Storage API    | Raw JSON保存・読取                          |
...
| raw_product_data           | Supabase Storage      | 大容量Raw JSON本体                    |
...
RawデータはDBではなくSupabase Storageへ保存する。
...
| 項目                | 保存先         | 内容                    |
| ------------------- | -------------- | ----------------------- |
| Raw JSON本体        | Supabase Storage | 外部APIレスポンス原本   |
...
Supabase Storage上のRawデータは、日付・API種別・hashを含むkeyで管理する。
...
| object storage       | local Storage API mock  | dev bucket         | prod bucket         |
...
| `SUPABASE_URL`         | api / reco / batch | API Hosting Secret / Reco Hosting Secret / GitHub Actions Secrets |
| `SUPABASE_SERVICE_ROLE_KEY` | api / reco / batch | API Hosting Secret / Reco Hosting Secret / GitHub Actions Secrets |
| `STORAGE_BUCKET_RAW`   | batch              | GitHub Actions Variables / Secrets                                |
...
| 最小権限にする                   | Supabase StorageやDB権限を必要範囲に限定する |
...
| batch          | object storage       | HTTPS / Storage API | 必須               |
...
| GitHub Actions | object storage       | HTTPS / Storage API | 必須               |
...
| batch → Supabase Storage       | service_roleを最小利用に限定する      |
...
| Supabase Storage Policy | Raw保存・読取に必要なbucket/prefixのみ                     |
...
| Supabase Storage障害     | Raw保存・再処理不可                     | Batchを失敗扱い、後で再実行           |
...
| Supabase Storage障害     | Online推薦には直接影響しない                        |
...
| 技術スタックとの整合 | Next.js / Express / FastAPI / Python / PostgreSQL / pgvector / Supabase Storageが反映されているか |
...
| Secret管理           | API Key / DB URL / Supabase KeyがGit管理外になっているか                              |
...
batch
↓ HTTPS / Storage API
Supabase Storage / Raw Product Data
↓
Database / Raw Metadata
↓
