...
| Webコンポーネント内の状態受け渡し   | UI内部実装のため                                           | 画面設計書 / フロントエンド詳細設計 |
...
| IF-DB-API-005    | Semantic設定参照                  | DB IF            | api                       | database                                   | SQL / Repository                        | semantic config                                                            | ○            | モジュール別入出力定義表     |
...
| IF-DB-RECO-001   | Config / Version参照              | DB IF            | reco                      | database                                   | SQL / Repository                        | semantic_config_version / model_version / ranking_config                   | ○            | モジュール別入出力定義表     |
| IF-DB-RECO-002   | 推薦候補商品参照                  | DB IF            | reco                      | database                                   | SQL / Repository                        | item / item_feature / item_semantic / popularity                           | ○            | モジュール別入出力定義表     |
| IF-DB-RECO-003   | Recommendation Run保存            | DB IF            | reco                      | database                                   | SQL / Repository                        | recommendation_run                                                         | ○            | モジュール別入出力定義表     |
| IF-DB-RECO-004   | Recommendation Result保存         | DB IF            | reco                      | database                                   | SQL / Repository                        | recommendation_result / result_item                                        | ○            | モジュール別入出力定義表     |
| IF-DB-RECO-005   | Reason保存                        | DB IF            | reco                      | database                                   | SQL / Repository                        | recommendation_reason                                                      | ○            | モジュール別入出力定義表     |
| IF-VEC-RECO-001  | Item Embedding検索                | Vector Store IF  | reco                      | pgvector                                   | SQL / vector search                     | query_embedding / item_embedding                                           | ○            | モジュール別入出力定義表     |
| IF-DB-BATCH-001  | Raw Metadata保存                  | DB IF            | batch                     | database                                   | SQL / Repository                        | raw_product_metadata                                                       | ○            | バッチ仕様書                 |
| IF-DB-BATCH-002  | Staging保存                       | DB IF            | batch                     | database                                   | SQL / Repository                        | staging_item / staging_item_image / staging_ranking_signal / staging_genre | ○            | バッチ仕様書                 |
| IF-DB-BATCH-003  | Product Diff保存                  | DB IF            | batch                     | database                                   | SQL / Repository                        | product_diff_result                                                        | ○            | バッチ仕様書                 |
| IF-DB-BATCH-004  | Item正本反映                      | DB IF            | batch                     | database                                   | SQL / Repository                        | item / item_image / item_review_summary                                    | ○            | バッチ仕様書                 |
| IF-DB-BATCH-005  | Ranking Snapshot反映              | DB IF            | batch                     | database                                   | SQL / Repository                        | ranking_snapshot / item_popularity_signal                                  | ○            | バッチ仕様書                 |
| IF-DB-BATCH-006  | Item有効状態更新                  | DB IF            | batch                     | database                                   | SQL / Repository                        | item.active_status / is_active                                             | ○            | バッチ仕様書                 |
| IF-DB-BATCH-007  | 商品意味生成キュー保存            | DB IF            | batch                     | database                                   | SQL / Repository                        | item_generation_queue                                                      | ○            | バッチ仕様書                 |
| IF-DB-BATCH-008  | Item Semantic保存                 | DB IF            | batch                     | database                                   | SQL / Repository                        | item_semantic                                                              | ○            | バッチ仕様書                 |
| IF-DB-BATCH-009  | Item Feature保存                  | DB IF            | batch                     | database                                   | SQL / Repository                        | item_feature / feature_input_hash                                          | ○            | バッチ仕様書                 |
| IF-DB-BATCH-010  | Feature正規化結果保存             | DB IF            | batch                     | database                                   | SQL / Repository                        | item_feature.normalized_value / item_meaning                               | ○            | バッチ仕様書                 |
| IF-VEC-BATCH-001 | Item Embedding保存                | Vector Store IF  | batch                     | pgvector                                   | SQL / Repository                        | item_embedding / embedding_input_hash                                      | ○            | バッチ仕様書                 |
| IF-DB-BATCH-011  | 分布メトリクス保存                | DB IF            | batch                     | database                                   | SQL / Repository                        | feature_distribution_metric / meaning_distribution_metric                  | ○            | バッチ仕様書                 |
| IF-DB-BATCH-012  | Import Summary保存                | DB IF            | batch                     | database                                   | SQL / Repository                        | item_import_summary                                                        | ○            | バッチ仕様書                 |
| IF-DB-BATCH-013  | Evaluation結果保存                | DB IF            | batch                     | database                                   | SQL / Repository                        | evaluation_run / evaluation_result / evaluation_metric                     | ○            | バッチ仕様書                 |
...
| IF-SHARED-003    | Offline EvaluationでのReco実行    | 共通ロジックIF   | batch                     | reco/shared logic または reco internal API | evaluation case / recommendation result | ○                                                                          | バッチ仕様書 |                              |
...
| レート制御                    | External API Rate Limiterで制御する                            |
...
| Workflow IF     | job失敗 / timeout / secrets不足                | `GRS-BAT-*` / `GRS-SYS-*`                            | GitHub Actions結果とbatch_run_logを突合可能にする |
...
| IF-DB-RECO-002  | candidate item ids                  | item / item_feature / item_semantic / popularity | Online推薦で参照                                                             |
...
| 対象                                             | 冪等キー                                                    |
| ------------------------------------------------ | ----------------------------------------------------------- |
| ranking_snapshot                                 | source / genre_id / period / fetched_at または snapshot_key |
