...
| IF-STG-001       | Raw JSON保存                      | Storage IF       | batch                     | Supabase Storage（S3 互換）                | REST PUT（SigV4 / `OBJECT_STORAGE_*`）  | raw product json                                                           | ○            | バッチ仕様書 / #1612         |
| IF-STG-002       | Raw JSON読取                      | Storage IF       | batch                     | Supabase Storage（S3 互換）                | REST GET（SigV4 / `OBJECT_STORAGE_*`）  | raw product json                                                           | ○            | バッチ仕様書 / #1612         |
...
| IF-DB-BATCH-012  | Feature入力hash保存               | DB IF            | batch                     | database                                   | SQL / Repository                        | item_feature_input（中間） / item_feature.feature_input_hash（最終は012）   | ○            | バッチ仕様書                 |
...
| IF-DB-BATCH-015  | Embedding入力hash保存             | DB IF            | batch                     | database                                   | SQL / Repository                        | item_embedding_input（中間） / item_embedding.embedding_input_hash（最終は015） | ○            | バッチ仕様書                 |
...
| IF-DB-BATCH-020  | Active Status Candidate保存       | DB IF            | batch                     | database                                   | SQL / Repository                        | item_active_status_candidate                                               | ○            | バッチ仕様書 / テーブル定義書 |
| IF-DB-BATCH-021  | Active Status Candidate読取・更新 | DB IF            | batch                     | database                                   | SQL / Repository                        | item_active_status_candidate                                               | ○            | バッチ仕様書 / テーブル定義書 |
...
| ---------- | ------------ | ---------- | -------------- | -------------- | -------------------------------- | ------------------------ | --------------------------------------------------------------------------- | ------- |
| IF-STG-001 | Raw JSON保存 | batch      | Supabase Storage（S3 互換） | REST PUT（SigV4） | raw json / object key / metadata | object key / save result | 楽天商品疑似差分取得Batch / 楽天既存商品再確認Batch / 楽天ジャンル同期Batch | ○       |
| IF-STG-002 | Raw JSON読取 | batch      | Supabase Storage（S3 互換） | REST GET（SigV4） | object key                       | raw json                 | Raw取込・Staging変換Batch                                                   | ○       |
...
| IF-DB-BATCH-012  | Feature入力hash保存    | batch      | database   | item_feature_input（BATCH-011） / item_feature.feature_input_hash（BATCH-012）                | INSERT / UPSERT           | ○       |
...
| IF-DB-BATCH-015  | Embedding入力hash保存  | batch      | database   | item_embedding_input（BATCH-014） / item_embedding.embedding_input_hash（BATCH-015）         | INSERT / UPSERT           | ○       |
...
| IF-DB-BATCH-020  | Active Status Candidate保存 | batch | database | item_active_status_candidate | INSERT / UPSERT（`candidate_status=detected`） | ○ |
| IF-DB-BATCH-021  | Active Status Candidate読取・更新 | batch | database | item_active_status_candidate | SELECT（`detected`） / UPDATE（`applied` / `superseded` / `discarded`） | ○ |

#### 11.3.1 Active Status Candidate IF（IF-DB-BATCH-020 / 021）

| 項目 | 方針 |
| ---- | ---- |
| 対象テーブル | `item_active_status_candidate` |
| Writer | BATCH-004（Item Active Status Candidate Resolver）。**IF-DB-BATCH-020** |
| Reader / Applier | BATCH-008（Item Active Status Updater）。**IF-DB-BATCH-021** |
| 冪等キー | `batch_run_id` + `source` + `external_item_code`（UNIQUE） |
| Item 本更新 | **本 IF では行わない**。`item.active_status` 更新は **IF-DB-BATCH-009** |
| Raw 非載荷 | 候補を `raw_product_metadata` に書かない（BATCH-004 §18.1.1） |
| 競合 | BATCH-008 は `product_diff_result`（IF-DB-BATCH-006）と本候補を両方読み、制限側優先（§18.1.1） |
| Online | 参照しない |
