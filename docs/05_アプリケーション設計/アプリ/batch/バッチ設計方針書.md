...
| Object Storage    | `OBJECT_STORAGE_ENDPOINT`、`OBJECT_STORAGE_ACCESS_KEY`、`OBJECT_STORAGE_SECRET_KEY`、`OBJECT_STORAGE_BUCKET`（製品=Supabase Storage / 接続方針 A） |
...
### 20.1 Config Version Resolver 連携（MOD-RECO-003）

batch パイプライン（BATCH-009〜015）では、Reco の **MOD-RECO-003 Config Version Resolver** と同一の解決ルールを用い、`semantic_config_version_id` / `model_version_id` 等を実行時に確定する。

| 項目 | 方針 |
| ---- | ---- |
| 正本 | `docs/06_実装設計/reco/MOD-RECO-003_Config Version Resolverモジュール仕様書.md`（§6.2 `BatchResolveContext`、§8.3.8 `generation_type` 別スコープ） |
| Queue との関係 | `item_generation_queue` 行は **version 列を持たない**（`item_generation_queue_テーブル定義書` §5）。BATCH-009 登録時に Resolver で解決した version は冪等キー・下位 Batch へのヒントとして利用し、永続化は `item_semantic` / `item_feature` / `item_embedding` 側 |
| 呼び出し契機 | BATCH-009（登録時の現行解決）、BATCH-010〜015（各ステップ開始時に `BatchResolveContext` を組み立てて解決） |
| 失敗時 | `GRS-CFG-*` を batch 側で `GRS-BAT-*` へ変換し、`item_generation_queue.queue_status = failed`（§14） |
| OL との差分 | batch では `ranking_config` / `reason_template` は **解決対象外**（OL 専用） |
