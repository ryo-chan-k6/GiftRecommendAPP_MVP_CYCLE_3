...
| period / lastBuildDate   | ranking_snapshotの観測条件・観測時点として保存                                |
...
| item_import_summary               | 取得・新規・更新・変更なし・失敗・snapshot件数を集計                                          |
...
| Ranking Snapshot       | source + external_genre_id + period + last_build_date                                  |
...
| Item Popularity Signal Writer    | ranking_snapshot_id + rankをキーに順位明細を全件反映    |
