...
| batch-rakuten-existing-item-recheck.yml | BATCH-004 / 005 / 006 / 007 / 008 / 017 | 既存商品再確認〜Item反映（設計上の複合想定名。実装差分は §19.2） |
...
```text
月曜〜土曜 00:30 JST は、UTCでは前日の日曜〜金曜 15:30に該当する。
...
```text
日曜 00:30 JST は、UTCでは土曜 15:30に該当する。
...
on:
  schedule:
    # Monday-Saturday 00:30 JST
    # UTC: Sunday-Friday 15:30
    - cron: "30 15 * * 0-5"
  workflow_dispatch:
    inputs:
      max_items:
...
| ----------------------------- | -------------------: | --------------- | ------------------------------ |
| batch-daily-orchestrator.yml  | 月曜〜土曜 00:30 JST | `30 15 * * 0-5` | 日次本線バッチ                 |
| batch-weekly-orchestrator.yml |       日曜 00:30 JST | `30 15 * * 6`   | 週次メンテナンス込み本線バッチ |
...
## 19. 実装差分（E1・2026-07-22）

本節は、設計イメージ（§7〜§9）と `develop` 上の実装の差分を正本として記録する。実装方針は [バッチ横串整合・本実装ギャップ一覧](./バッチ横串整合・本実装ギャップ一覧.md) §5 **案 A（複合子）**（Human 確定）。

### 19.1 実装済みファイル

| 種別 | ファイル | 内容 |
| ---- | -------- | ---- |
| 親 | `batch-daily-orchestrator.yml` | 日次本線。`workflow_dispatch`。Phase1 は `schedule` 無効（コメント保持） |
| 親 | `batch-weekly-orchestrator.yml` | 週次本線（日次相当内包）。Phase1 は `schedule` 無効 |
| 親 | `batch-manual-orchestrator.yml` | 手動シナリオ |
| 複合子 | `batch-rakuten-item-import.yml` | 003 → 005 → 006 → 007 → 008 → 017 |
| 複合子 | `batch-item-meaning-generation.yml` | 009 → 010 → 011 → 012 → 013 → 014 → 015 → 017 |
| 複合子 | `batch-retry-failed-items.yml` | 010 → 011 → 012 → 013 → 014 → 015 |
| 複合子 | `batch-rakuten-existing-item-pipeline.yml` | 004 → 005 → 006 → 007 → 008 → 017（週次用） |

### 19.2 `batch-rakuten-existing-item-recheck.yml` の扱い

| 項目 | 内容 |
| ---- | ---- |
| 設計書 §3.2 の表記 | 004/005/006/007/008/017 複合子としての想定名 |
| 実装の事実 | **BATCH-004 独立子**として縦串で先行確定済み |
| E1 方針 | 独立子ファイルをリネーム・破壊しない。週次の連鎖は `batch-rakuten-existing-item-pipeline.yml` を新設 |
| 週次親の呼び出し | `existing_item_pipeline` job が pipeline 複合子を `uses` |
...
| ---- | ---- |
| 日次本線 | 載せない |
| 週次 | `run_offline_evaluation` default `false`（任意のみ） |
| 手動 | `offline_evaluation` / `feedback_analysis` シナリオあり（本線ではない。MVP △ scaffold） |

### 19.4 Phase1 運用

```text
- 親の on.schedule はコメントアウト（無効）
- 検証は workflow_dispatch で行う
- Phase2 で daily cron、Phase3 で weekly cron を有効化する（§16）
```
