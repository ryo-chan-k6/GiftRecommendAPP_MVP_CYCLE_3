...
| 状態      | 意味                                              | 終端状態 |
| --------- | ------------------------------------------------- | -------- |
| accepted  | recoが`recommendation_run`をINSERTし、パイプライン実行待ちの状態 | ×        |
...
```mermaid
stateDiagram-v2
    [*] --> accepted: recoがRun INSERT（MOD-RECO-002）
...
| 現状態   | 次状態    | 契機                                                | 更新主体   | 備考                                    |
| -------- | --------- | --------------------------------------------------- | ---------- | --------------------------------------- |
| なし     | accepted  | recoが`MOD-RECO-003`解決後に`MOD-RECO-002`で`recommendation_run`をINSERT | reco       | apiは`recommendation_request`保存のみ（`MOD-RECO-002` §16.2） |
