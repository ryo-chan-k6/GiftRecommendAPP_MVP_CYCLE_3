...
| Reason生成失敗     | Run: failed または Result生成済みReasonなし | MVPではResult全体失敗扱いを基本                               |
...
| Recommendation Run Phase | recommendation_run_phase_log / phase_log | phase_status              |
...
| API                                        | 状態観点                                             |
| ------------------------------------------ | ---------------------------------------------------- |
| POST /recommendations                      | loading / succeeded / empty / failedをUIへ返せること |
| POST /recommendations/{result_id}/feedback | submitted / invalid / ignoredを扱えること            |
| GET /masters/relationships                 | 状態管理対象外。is_activeで制御                      |
| GET /masters/occasions                     | 状態管理対象外。is_activeで制御                      |
