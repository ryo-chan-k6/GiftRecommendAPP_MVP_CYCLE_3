...
| 画面・UI               | Product Detail Page State           | web                     | web                | user                    | 画面状態                   | OL             | ○       | Result Snapshotを表示                     |
...
| 外部商品データ         | Item Active Status Candidate        | batch                   | batch              | batch                   | 一時 / 判定結果            | BT             | ○       | BATCH-004 Writer / BATCH-008 Applier      |
...
| Item Active Status Candidate | batch             | batch       | batch          | batch            | 一時 / 判定結果 | active_status 候補      |
...
| API                                          | 主な入出力リソース                             | 主責務     |
| -------------------------------------------- | ---------------------------------------------- | ---------- |
| `POST /api/v1/recommendations`                             | Recommendation Request / Recommendation Result | api        |
| `POST /api/v1/recommendation-results/{resultId}/feedback` | Recommendation Feedback                        | api        |
| `GET /api/v1/masters/relationships`                        | Relationship Master                        | api        |
| `GET /api/v1/masters/occasions`                            | Occasion Master                            | api        |
| `GET /api/v1/masters/semantic-configs`                     | Semantic Config                            | api        |
| `GET /api/v1/masters/feature-rules`                        | Feature Rule                               | api        |
...
| 既存商品再確認Batch       | Item / Rakuten Item Search Response / Item Active Status Candidate | batch  |
