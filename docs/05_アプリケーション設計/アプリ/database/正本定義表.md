...
| 画面・UI               | Product Detail Page State           | 画面状態                  | web                     | web                 | web                | 上書き可                | 可                    | ○       | 商品詳細画面状態             |
...
| 外部商品データ         | Item Active Status Candidate        | 派生 / 一時               | batch                   | database            | batch              | Upsert可                | 可                    | ○       | BATCH-004 候補 / BATCH-008 適用 |
...
| Item Active Status Candidate | 派生 / 一時         | batch            | active_status 候補    |
...
| -------------------------------------------- | ----------------------- | --------------------- | ---------------------------- |
| `POST /api/v1/recommendations`                             | Recommendation Request  | Recommendation Result | Request / Resultを内部正本化 |
| `POST /api/v1/recommendation-results/{resultId}/feedback` | Recommendation Feedback | 保存結果              | Feedbackを内部正本化         |
| `GET /api/v1/masters/relationships`                        | なし                    | Relationship Master   | 設定正本参照                 |
| `GET /api/v1/masters/occasions`                            | なし                    | Occasion Master       | 設定正本参照                 |
| `GET /api/v1/masters/semantic-configs`                     | なし                    | Semantic Config       | 設定正本参照                 |
| `GET /api/v1/masters/feature-rules`                        | なし                    | Feature Rule          | 設定正本参照                 |
...
| 楽天既存商品再確認Batch   | Raw Product JSON / Raw Product Metadata / Item Active Status Candidate | Raw / Metadata / 派生 |
| 商品有効状態更新Batch     | Item Active Status / Item Active Status Candidate                | 状態 / 派生               |
