...
| Item Active Status Candidate       | item_active_status_candidate        | 外部商品データ系         | 派生 / 一時                 | batch                         | BT        | ○       | BATCH-004 再確認の active_status 候補。BATCH-008 が適用 |
...
> **出所列**: `source` / `source_api` は **行に持たない**（`item_review_summary` と同型）。マーケットは `item.source`、取得 API は楽天商品検索 API（`item_search`）をテーブル責務で暗黙。監査時は `raw_product_metadata.source_api` を参照。
...
| 既存商品再確認Batch       | Item / Rakuten Item Search Response / Item Active Status Candidate                |
...
| 商品有効状態更新Batch     | Item / Item Active Status / Item Active Status Candidate                          |
