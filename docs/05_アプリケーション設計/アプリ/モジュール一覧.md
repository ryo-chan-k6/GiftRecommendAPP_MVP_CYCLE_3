...
| `MOD-RECO-012` | Candidate Retriever | Candidate Retriever | 推薦 | reco | OL | ○ | 内部で Pre Hard Filter と Vector Retrieval を順に実行し候補商品を抽出する |
...
| `MOD-RECO-012` | Candidate Retriever | Candidate Retriever | OL | ○ | Pre Hard Filter（内部）と Vector Retrieval で候補商品を抽出する |
...
flowchart TD
    ...
    J --> K[Candidate Retriever]
    K --> M[Post Hard Filter Executor]
    M --> N[Feature Matcher]
...
`Candidate Retriever`（`MOD-RECO-012`）は Orchestrator から **1 回**呼び出され、内部で `pre_hard_filter` → `retrieval` を順に実行する（`MOD-RECO-012` モジュール仕様書 §2・§8）。

---

### 8.3 Pre Hard Filter / Retrieval / Post Hard Filterの責務分離

| モジュール | サブフェーズ | 目的 | 主な入力 | 主な出力 |
| ---------- | ------------ | ---- | -------- | -------- |
| Candidate Retriever（`MOD-RECO-012`） | `pre_hard_filter` | Retrieval前に検索対象を削減する | Recommendation Request / budget / ng / item metadata | pre_filtered_item_pool |
| Candidate Retriever（`MOD-RECO-012`） | `retrieval` | 候補商品を意味的に抽出する | pre_filtered_item_pool / query_embedding / item_embedding | retrieval_candidate |
| Post Hard Filter Executor（`MOD-RECO-013`） | — | Retrieval後に意味的NG・重複・不整合を除外する | retrieval_candidate / semantic result / item_semantic | validated_retrieval_candidate |
...
| `MOD-RECO-019` | Final Score Calculator | ○ |
...
| Candidate Retriever（`pre_hard_filter`） | 予算、販売状態、明確なNG、画像有無、URL有無           |
| Candidate Retriever（`retrieval`）       | Embedding検索、キーワード検索、Hybrid検索             |
...
| Risk Scorer                          | 贈答リスク補正                                        |
| Final Score Calculator               | context_score / popularity_score / risk_penalty から final_score 算出 |
| Final Ranker                         | MMR、多様性調整                                       |
