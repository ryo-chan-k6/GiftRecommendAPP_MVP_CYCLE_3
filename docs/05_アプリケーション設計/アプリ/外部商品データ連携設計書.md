...
| `page`          | ページ番号           | 取得上限内で制御 |
| `period`        | ランキング期間（楽天クエリ） | **MVP では原則送出しない**。ドメイン観測キーの `daily` とは別物（BATCH-002 §6.2.1）。`realtime` のみ送出可 |
...
**MVP 方針（#1603 / #1606）:** Adapter（`adapt_genre_raw_payload`）は現行キー **`genre` のみ**を受理する。旧 `current` は後方互換として残さない。
...
| 4   | Object Storageの具体サービス                             | Supabase Storage / S3 等の採用先に合わせて物理設計で確定する                |
