...
`モジュール一覧.md` では、API / Reco / Batch の実行機能を構成するモジュールを整理している。  
...
| モジュールID | モジュール名 | 物理名 | 分類 | 主責務 | 処理種別 | MVP対象 |
|---|---|---|---|---|---|---:|
| `MOD-RECO-001` | 推薦実行制御 | Recommendation Orchestrator | 実行制御 | 推薦パイプライン全体の実行順序を制御する | OL | ○ |
| `MOD-RECO-003` | Config / Version解決 | Config Version Resolver | 設定解決 | 利用する設定・モデルVersionを決定する | 共通 | ○ |
| `MOD-RECO-004` | Semantic抽出 | User Semantic Extractor | User Meaning | ユーザー入力からSemantic Conceptを抽出する | OL | ○ |
...
| `MOD-RECO-016` | 文脈スコア算出 | Context Scorer | Matching | context_scoreを算出する | OL | ○ |
| `MOD-RECO-017` | 人気補正算出 | Popularity Scorer | Ranking | popularity_scoreを算出する | OL | ○ |
| `MOD-RECO-018` | リスク補正算出 | Risk Scorer | Ranking | risk_penaltyを算出する | OL | ○ |
| `MOD-RECO-019` | 最終スコア算出 | Final Score Calculator | Ranking | final_scoreを算出する | OL | ○ |
| `MOD-RECO-020` | 最終順位生成 | Final Ranker | Ranking | 表示順位rankを決定する | OL | ○ |
| `MOD-RECO-021` | Recommendation Result生成 | Recommendation Result Builder | 出力処理 | recommendation_result / result_itemを生成する | OL | ○ |
| `MOD-RECO-022` | Result Snapshot生成 | Result Snapshot Builder | 出力処理 | 表示時点の商品名、価格、URL、画像URL等をSnapshot化する | OL | ○ |
| `MOD-RECO-023` | Reason生成 | Reason Generator | 出力処理 | 推薦理由を生成する | OL | ○ |
| `MOD-RECO-002` | Recommendation Run記録 | Recommendation Run Recorder | ログ・観測 | 推薦実行単位を記録する | OL | ○ |
...
| 処理種別 | OL |
| MVP対象 | ○ |
| 主な入力 | ranked_items / item current values |
| 主な出力 | result item snapshot |
| 関連定義 | Recommendation Result定義書 |

### 主責務

- 表示時点の商品名、価格、URL、画像URLをSnapshot化する
- 推薦結果表示後にItem正本が更新されても、結果表示の再現性を担保する
- Recommendation Result生成とReason生成へ表示用の商品情報を渡す
...
| 処理種別 | OL |
| MVP対象 | ○ |
| 主な入力 | exception / phase context / request_id |
| 主な出力 | standardized reco error / error_log input |

### 主責務

- reco内部の例外を推薦ドメイン向けの標準エラーへ変換する
- Phase Log / Error Logへ渡す失敗情報を整える
- apiへ返すエラー種別とメッセージ粒度を制御する
