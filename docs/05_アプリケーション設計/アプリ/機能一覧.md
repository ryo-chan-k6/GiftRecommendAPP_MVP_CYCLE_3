...
| 保存先 | Supabase Storage（S3 互換 API・案 A）。env 正本は `OBJECT_STORAGE_*` |
...
| Pre Hard Filter | Candidate Retriever（`pre_hard_filter` サブモジュール） |
| Retrieval | Candidate Retriever（`retrieval` サブモジュール） |
