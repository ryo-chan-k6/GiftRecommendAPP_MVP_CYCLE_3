...
| テスト環境設計書       | テスト環境（Layer1〜3）、Layer2 workflow 一覧、Agent 運用対応表（§2.6） |
...
全テストレベルに対する **AI Agent 運用対応**（実行経路・Layer・Epic 関係）は [テスト環境設計書.md](./テスト環境設計書.md) §2.6 を正本とする。Epic C Epic Definition `agent_test_operations.level_map` の根拠は本 §4.1 である。
...
### 9.5.3 Layer2 `test-system.yml` の API E2E 条件（Epic C C3 / follow-up #1426）

Layer2 システムテスト workflow（`.github/workflows/test-system.yml`）は **GHA 内 ephemeral DB + Redis 完結**を正とする（cloud dev URL 入力は将来拡張）。

| 区分 | Phase4b 最小 API 整備前（履歴） | Phase4b 最小 API 整備後（現行） |
| ---- | ------------------------- | ------------------------ |
| 必須 pass | fixture 索引、PostgreSQL 接続、test seed 行、Redis PING | 同上 + API health / RecommendationRun |
| skip 可 | `GET /api/v1/health`、`GET /internal/reco/v1/health`、`POST /api/v1/recommendations` | なし（infra 切り分け時のみ `skip_api_e2e=true`） |
| workflow 入力 | `skip_api_e2e=true`（当時の既定） | `skip_api_e2e=false`（既定） |
| 失敗扱い | infra / fixture 系のみ | API 導線含む全ケース |
| 成果物 | `tests/e2e/results/system-test-report.json`（artifact / job summary） | 同上 |

Phase4b 最小 API 整備後は `apps/api` / `apps/reco` を GHA 上で起動し、API E2E を必須 pass とする。`skip_api_e2e=true` は DB / Redis / fixture のみ切り分ける場合の例外入力である。artifact の `phase4bPending` は skip 有効時のみ `true` になる（互換フィールド）。
