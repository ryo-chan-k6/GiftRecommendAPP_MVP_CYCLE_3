...
| 横断Epic（`api-contract-generated`） | 1c Orval設定 + generated生成 | `contract-definition` | 1b | generated一括生成は集約（並列不可） |

- Contract Gate通過（契約仕様書＋OpenAPI＋generatedの親Epic Branchマージ）を Phase4b 実装Taskの前提にする。
