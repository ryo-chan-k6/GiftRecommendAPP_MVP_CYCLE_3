...
- 物理ER → enum/コード定義 → テーブル定義書 → DDL → `supabase/migrations` / `supabase/seeds` → データ保持・削除方針書。
...
| A4 `seed-strategy` | `supabase/seeds/masters` + `config.toml` [db.seed] + 最小 seed script | A3 | A5 前 |
