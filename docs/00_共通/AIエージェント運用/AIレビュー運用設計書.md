...
| Review Definition | `prompts/definitions/reviews/<workstream>/pr-review.yaml`（または Task Definition と同ディレクトリの `pr-review.yaml`） | AIレビューの観点・対象・結果方針 |
...
2. Task Definition と同ディレクトリの `pr-review.yaml`（sibling）
3. workstream 規約パス `prompts/definitions/reviews/<workstream>/pr-review.yaml`（workstream は `tasks/<workstream>/` と一致）
4. Issue 番号・PR 番号・branch summary・`target` によるスコアリング走査

したがって、Review Definition は **対象Taskと同じ workstream に作成し、Task Branch（PR head）へ commit して PR の変更ファイルに含める**。
...
- `/create-pr` は、`ai_review_required: true` かつ Review Definition が PR head（変更ファイル）からも規約パスからも解決できない場合、PR作成を停止する（正本: [create-pr.md](../../../.cursor/commands/create-pr.md) 停止条件）。
