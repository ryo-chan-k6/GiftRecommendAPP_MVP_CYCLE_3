...
| 判定基準（#1533 Human 確定） | Reco 内部 soft **1,500ms** / hard **2,000ms**。同期外部 AI 込み soft **6,000ms** / hard **8,000ms**。`phase_output` は Phase3 案出し → 正式反映は Epic #1532 |
...
| 正式反映（Ranking・User Meaning） | S4 後 | #1533 で soft/hard・主要フェーズ確定。`phase_output` のみ未確定宣言 | #1533 CLOSED（Epic #1532 で develop 取込） |
| **Phase3** | S3（live・Reason 込み） | Reason 込み E2E、`phase_output` 上限案、分離判定 | 完了（#1535 / #1538 develop 着） |
| 正式反映（phase_output） | Phase3 後 | §13.2 `phase_output` / 性能要件の Reason 関連 | Epic #1532 で追従（案 A: soft 3s / hard 7s・Human Review） |
...
| 正式反映 Task（soft/hard・主要フェーズ） | #1533 CLOSED（`phase_output` 未確定を宣言） |
| Phase3 Epic / Task | #1535 / #1536 CLOSED（#1538 develop） |
| 正式反映 Epic（#1533 + phase_output） | #1532 |
...
1. Epic #1532 で #1533 確定値を develop へ取り込む
2. Human Review で `phase_output` 案 A（soft 3,000ms / hard 7,000ms）の採否を確定（計測定義注意あり）
3. Reason 込み E2E を同期外部 AI 込み 6s/8s と同一枠にするか確定
4. 必要なら `phase_output` 計測定義修正（`response_built` 除外）を別 Task 化
...
| 2026-07-22 | Phase3 develop 着（#1538）。正式 docs を #1532 に集約する旨を更新 |
