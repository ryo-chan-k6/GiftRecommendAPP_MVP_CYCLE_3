...
| 観点 | 内容 |
| ---- | ---- |
| 主な確認内容 | 項目、欠損、ページング、rate limit、エラー形式、応答時間 |
| 判断結果の反映先 | 外部商品データ連携、Batch仕様、テーブル設計 |
...
- BATCH 商品取込系 Epic とスケジュールを調整する。
- fixture 候補は `tests/fixtures/` への後続反映を検討する（本TVの out of scope でも可）。
...
| 指標 | リクエスト単位の応答時間（wall-clock）。複数回実行時は min / avg / p50 / p95 を記録 |
| 記録 | リクエスト要約、応答 schema / 欠損、応答時間、エラー形式。secret 実値は記録しない |
