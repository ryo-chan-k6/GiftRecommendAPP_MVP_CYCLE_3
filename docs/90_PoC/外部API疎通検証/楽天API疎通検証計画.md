...
| 制約 | 方針 |
| ---- | ---- |
| 目標 QPS | **8**（アプリ登録上限 10 の下。ハードキャップ 10 を超えない） |
| 接続元 IP | 楽天側に登録した外部 IP からのみ。ハーネスで **照合必須** |
| CI live | **禁止**（GitHub-hosted は動的 IP） |
| Rate Limiter 本実装 | #1603 外。ハーネスは最小間隔のみ。`MOD-BATCH-008` は別 Task |
...
| 間隔 | 既定 `RAKUTEN_MAX_QPS=8` → 最小間隔 125ms。`RAKUTEN_MIN_INTERVAL_MS` で上書き可（ただし 100ms 未満＝10 QPS 超は拒否） |
