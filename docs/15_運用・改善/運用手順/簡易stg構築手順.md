...
| 関連 Issue | #1686（手順正本化）。親 #144 / #1773（東京リージョン前提・移行手順の追加。親 #1747） |
...
stg への適用は **Supabase CLI（`supabase db push`）を正**とする。CI job 等による自動適用は別運用・将来拡張であり、本手順の対象外とする。ローカル向け手順は [DB構築手順書](../../06_実装設計/database/DB構築手順書.md) を参照。
...
投入例（`STG_SEED_DB_URL` は §6.1 の「master seed 投入」用の一時環境変数。Direct または Session pooler の接続文字列を設定し、コマンド履歴・docs に実値を残さない）:

```bash
for f in $(ls supabase/seeds/masters/*.sql | sort); do
  psql "$STG_SEED_DB_URL" -v ON_ERROR_STOP=1 -f "$f"
done
```
