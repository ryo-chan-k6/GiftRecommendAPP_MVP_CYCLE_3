...
- **拡大候補（追加ジャンル）:** ジャンル地図完了後の追加ID・第1波・BATCH-002/003優先は [拡大候補 Decision Log](../../../ai-logs/human-decisions/2026-08-04-batch-fetch-plan-expansion-candidates.md)（§10 No.12 / §11.6）を正とする。**MVP 4IDは置き換えない（追加のみ）**。BATCH-003 genre は明示指定のみ（子・孫自動展開なし）。Human採択前は運用既定にしない。
...
| 12 | BATCH-002/003 fetch_plan拡大候補 | 追加ジャンルID・第1波・002/003優先・明示指定のみ | **Human採択待ち（2026-08-04）**。[拡大候補 Decision Log](../../../ai-logs/human-decisions/2026-08-04-batch-fetch-plan-expansion-candidates.md)。MVP 4IDは維持（追加のみ）。子・孫自動展開なし。AI推奨第1波: `101381` / `551167` / `510901` / `216129`。BATCH-003優先。詳細は§11.6 |

採択の正本は[楽天Fetch運用値 Human Decision Log](../../../ai-logs/human-decisions/2026-07-30-rakuten-fetch-ops-policy.md)とする。No.1の具体的 `fetch_plan` は [2026-07-31 Log](../../../ai-logs/human-decisions/2026-07-31-rakuten-fetch-mvp-fetch-plan.md) で承認済み。local 実楽天HTTP（パターンB）は [検証結果](./楽天Fetch_local_live検証結果_1765.md) で実施済み。secret投入・追加実行は引き続き Human 環境での判断とする。GHA楽天HTTP live化は当面禁止のまま（No.7）。本格収集の段階・期間上限は No.10 / [本格収集運用枠 Log](../../../ai-logs/human-decisions/2026-07-31-batch-data-collect-ops-plan.md) を正とする。ジャンル地図キャンペーン枠（No.11）は [2026-08-03 Log](../../../ai-logs/human-decisions/2026-08-03-batch-genre-map-campaign-ops-plan.md) が **`decided`**。fetch_plan拡大候補（No.12）は [2026-08-04 Log](../../../ai-logs/human-decisions/2026-08-04-batch-fetch-plan-expansion-candidates.md) が **Human採択待ち**。採択後に fetch_plan反映・段階収集の別Taskへ進む。
...
- [ジャンル地図キャンペーン運用枠 Decision Log](../../../ai-logs/human-decisions/2026-08-03-batch-genre-map-campaign-ops-plan.md) が **`decided`** である（#1829）
- 目的は楽天ジャンル全体像（ID・階層・件数）の段階把握。**MVP fetch_plan 4ID（`100000` / `100003` / `100004` / `100005`）は置き換えない**（拡大情報源）
- **採択枠:**
  - 起点: root `0` からの BFS（未展開ジャンルを次 Run の `--genre-ids` へ、最大20）
  - 完了: **全階層取り切り**（キュー空）。**depth 上限なし**
  - 1 Run 上限: **20 genre**（起点ID数）
  - QPS: 常用 **2** 維持。キャンペーン BATCH-001 は安全側 **`max_qps=1`**。商品収集 cron と同時 live 禁止
  - 容量ゲート: soft（80%）→**Slack通知**、hard→**自動停止＋Slack**。毎Runの人手容量確認はしない
  - hard 初期値: rows=100,000 / api_calls=100,000 / runs=5,000 / raw=5 GiB / queue=50,000
  - 停止: 429連続 / `paused`増 / hard上限 / Human中断（＋egress不一致・secret疑い・同時live検知）
...
- [BATCH-002/003 拡大候補 Decision Log](../../../ai-logs/human-decisions/2026-08-04-batch-fetch-plan-expansion-candidates.md) が着手ゲート（#1844）。状態は **Human採択待ち**（AI推奨提示済み）
...
- **AI推奨（採択前・正本は Decision Log）:**
  - 第1波追加ID: `101381`（カタログギフト・チケット） / `551167`（スイーツ・お菓子） / `510901`（日本酒・焼酎） / `216129`（ジュエリー・アクセサリー）
  - BATCH-003（商品Item）優先。BATCH-002 は 003 smoke 後に新IDを `max_pages=1` で必要最小
  - Run予算ノブ（§5.3.4）・監視閾値（§5.3.5）・QPS・同時live1本は変更しない。総量増は Run回数で吸収
- **後続:** Human採択（`decided`）後に fetch_plan反映・段階収集を別Task化。拡大 live は Human。本節は crontab を変更しない
...
| [BATCH-002/003 拡大候補 Human Decision Log](../../../ai-logs/human-decisions/2026-08-04-batch-fetch-plan-expansion-candidates.md) | §10 No.12 / §11.6。Human採択待ち。追加ID・第1波・003優先・明示指定境界 |
