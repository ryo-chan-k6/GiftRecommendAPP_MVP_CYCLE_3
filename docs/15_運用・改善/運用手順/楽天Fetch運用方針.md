...
| 安全側QPS | **1を推奨・任意** | 長時間Run・連続再実行での採否は§10でHuman判断 |
...
| BATCH-003 | **事業上の取得打ち切り上限は設けない**（§5.3）。承認済みスコープを cursor で継続走査し、`exhausted` まで進める | **Run単位**の `pages_per_run` / 消費cursor数 / 実行時間。恒久の深さ打ち切りではない | 1ルート × 1カーソル × 1ページ。疎通確認時は `hits=3` | 対象ジャンル、Run予算の初期値、監視閾値 |
| BATCH-004 | 優先度付き部分集合（全activeは週次既定にしない） | 件数/Run・件数/週 | `max_items=1` | 100 / 500 / 1000件の採用段階 |

BATCH-002の `max_pages=1`（〜2）は現行設計のまま維持する。
BATCH-003の「運用予算」はHuman採択済みの初期値（§5.3.4 / §5.3.5、2026-07-30）である。ただし現行workflowをlive化する承認ではなく、値の実装反映・workflow変更は後続live実装Task（§11.1）で行う。

### 5.3 BATCH-003: 継続取得と運用予算の分離

#### 5.3.1 Human方針（2026-07-30）
...
| 概念 | 意味 | 使い方 |
| ---- | ---- | ------ |
| カタログ深さ上限 | あるジャンルを page N で永久打ち切り | **採用しない**（事業方針） |
...
| ノブ | smoke | 初期live（最初の3〜5 Run） | 通常継続（既定） | 加速（安定後・任意） | コスト抑制 |
| ---- | ----: | -------------------------: | ---------------: | -------------------: | ---------: |
...
- 既定は **通常継続**（`pages_per_run=60` / `cursors_per_run=1` / route 1本 / `hits=30` / 45分）。
...
実容量の絶対値はプラン確定前のため、比率・増分・エラー率を初期採用する。**運用開始1週間の実測後に数値を見直す**前提とする。
...
| 2 | BATCH-003カタログ深さ | 深さ打ち切りあり / **なしで継続取得** | **Human方針採択（2026-07-30）**: 事業上の取得打ち切り上限は設けない。QPS遵守で継続し `exhausted` まで進める |
| 2b | BATCH-003のRun予算 | pages/cursor/時間の初期値 | **Human採択（2026-07-30）**: §5.3.4。既定 `pages_per_run=60` / `cursors_per_run=1` / route 1本 / `hits=30` / 45分。立ち上げは10から段階拡張 |
| 2c | 監視閾値 | DB / Storage / GHA / 429 | **Human採択（2026-07-30）**: §5.3.5。初期は比率・増分・エラー率ベース。運用1週間後に実測で見直す |
...
| 8 | 安全側QPS=1 | 全Runの既定 / 長時間・再開時のみ / 不採用 | **判断待ち**。長時間Run、BATCH-003/004、429後の再開時のみQPS=1（推奨） |
| 9 | クールダウン | 15分 / 30分 / 60分 / 別値 | **判断待ち**。初回15分、再発時60分以上（推奨） |

No.2の深さ方針、No.2b / 2cのRun予算・監視閾値の初期値はHuman採択済み（2026-07-30）。数値はlive実装後の実測で見直す前提であり、live実装・workflow変更は別Taskで行う。
...
- §10のHuman判断が完了している（BATCH-003深さ方針は§5.3反映済み）
- BATCH-003について、Run予算とカタログ深さ打ち切りを混同しない実装になっている
- 採用したRun予算・再開方式・監視指標がTask Definitionへ反映されている
...
| 2026-07-30 | BATCH-003を「深さ打ち切りなし＋Run予算で監視調整」に改訂。BATCH-002の `max_pages` は維持 |
| 2026-07-30 | BATCH-003のRun予算初期値（§5.3.4）と監視閾値初期値（§5.3.5）をHuman採択。実測後に見直す前提 |
