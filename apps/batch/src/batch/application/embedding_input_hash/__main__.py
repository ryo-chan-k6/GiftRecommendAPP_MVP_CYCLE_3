...
def main(argv: list[str] | None = None) -> int:
    ...
    if not is_live_db_reader(db_reader):
        print(
            "DATABASE_URL is required for non --scaffold-demo BATCH-014 "
            "(DbReader postgres backend). Use --scaffold-demo for local/CI.",
            file=sys.stderr,
        )
        return 2

    repos = EmbeddingInputHashRepositories(db_writer=db_writer, db_reader=db_reader)
    job = EmbeddingInputHashJob(repositories=repos)
    result = job.run(
        job_run_id=args.job_run_id,
        max_items=args.max_items,
        source=args.source,
    ...
    print(
        f"BATCH-014 status={result.status} "
