"""MOD-RECO-017 Popularity Scorer executor unit tests (module spec §14 unit)."""

from __future__ import annotations

import json

import pytest

from conftest import (
    _sample_context,
    build_review_repository,
    build_scorer,
)
from reco.application.popularity_scorer import (
    MODULE_ID,
    PHASE_NAME,
    PopularityScorerError,
    SURFACE_ERROR_CODE,
)
from reco.infrastructure.logger.logger import ScaffoldRecoLogger


# §14 No.12 context_score_result 欠損
def test_execute_raises_grs_rec_012_when_context_score_result_missing() -> None:
    context = _sample_context()
    del context.context_score_result  # type: ignore[attr-defined]
    scorer = build_scorer()

    with pytest.raises(PopularityScorerError) as exc_info:
        scorer.execute(context)

    assert exc_info.value.error_code == SURFACE_ERROR_CODE


def test_execute_raises_grs_rec_012_when_review_repository_fails() -> None:
    context = _sample_context()
    repository = build_review_repository(should_fail_on_fetch=True)
    scorer = build_scorer(repository=repository)

    with pytest.raises(PopularityScorerError) as exc_info:
        scorer.execute(context)

    assert exc_info.value.error_code == SURFACE_ERROR_CODE


def test_execute_attaches_popularity_score_result_and_metrics_to_execution_context() -> None:
    context = _sample_context(run_id="run-popularity-scorer-handoff")
    scorer = build_scorer()

    result_context = scorer.execute(context)

    result = result_context.popularity_score_result  # type: ignore[attr-defined]
    assert result.total_scored == 1
    assert result_context.popularity_scorer_candidate_count == 1  # type: ignore[attr-defined]
    assert result_context.popularity_scorer_latency_ms is not None  # type: ignore[attr-defined]
    assert result_context.popularity_missing_signal_count is not None  # type: ignore[attr-defined]
    assert result_context.popularity_score_value_out_of_range_count is not None  # type: ignore[attr-defined]
    assert MODULE_ID in result_context.completed_modules


# §14 No.18 ログ
def test_execute_emits_structured_log_with_trace_id_without_secrets() -> None:
    context = _sample_context(
        run_id="run-popularity-scorer-log",
        trace_id="trace-mod-reco-017-unit",
    )
    logger = ScaffoldRecoLogger()
    scorer = build_scorer(logger=logger)

    scorer.execute(context)

    completion_logs = [record for record in logger.records if record.event == PHASE_NAME]
    assert len(completion_logs) == 1
    log_record = completion_logs[0]
    assert log_record.context.trace_id == "trace-mod-reco-017-unit"
    assert log_record.context.run_id == "run-popularity-scorer-log"
    assert log_record.attributes["popularity_scorer_candidate_count"] == 1
    assert log_record.attributes["module_id"] == MODULE_ID
    assert log_record.attributes["total_scored"] == 1
    assert log_record.attributes["max_review_count_in_candidates"] == 120
    serialized = json.dumps(log_record.attributes, ensure_ascii=False).lower()
    assert "api_key" not in serialized
    assert "password" not in serialized
    assert "token" not in serialized
    assert "secret" not in serialized
